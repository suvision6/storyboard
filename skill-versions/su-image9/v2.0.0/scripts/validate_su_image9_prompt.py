#!/usr/bin/env python3
"""Validator/compiler for su-image9 v2.0.0 final prompts."""
...
def require_text(page: Page, text: str, source: str, check: str = "G0-04") -> None:
    ...
    if normalize_for_compare(text) not in normalize_for_compare(page_text):
        page.findings.append(issue(check, "fail", f"{page.name}/{source}", "missing", "required v2.0.0 generation-layer text missing"))
...
def validate_g0(pages: list[Page], compiled_text: str, panel_plan: dict[str, Any]) -> None:
    ...
    for page in pages:
        ...
        if re.search(r"(?m)^(STYLE_LOCK|CANVAS_LOCK|REFERENCE_LOCK|PANEL_TASKS|NEGATIVE_LOCK):", page.text):
            page.findings.append(issue("G0-10", "fail", page.name, "v1.7.3 structure", "old PANEL_TASKS/canon-lock structure is not allowed for v2.0.0 final prompts"))
    ...
    for page in pages:
        ...
        for index in range(1, 10):
            ...
            for field_name in MACHINE_PANEL_FIELDS:
                if pp.get(field_name) in {None, ""}:
                    page.findings.append(issue("G1-02", "fail", f"{page.name}/PANEL-{index}/{field_name}", "missing", "required v2.0.0 machine-track field missing"))
        ...
        if (pos and neg) or (not pos and not neg):
            ...
            if pplan.get("anchor_decision") != "human_confirmed":
                page.findings.append(issue("G1-11", "fail", f"{page.name}/PANEL-1", tag, "panel_plan missing anchor_decision=human_confirmed"))
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compile and validate su-image9 v2.0.0 prompts.")
