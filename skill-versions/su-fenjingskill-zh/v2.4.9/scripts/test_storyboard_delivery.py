...
RULE_REVISION_245 = "2.4.5-director-shot-integrity-guard-2026-07-18"
RULE_REVISION_246 = "2.4.6-dialogue-partition-guard-2026-07-20"
RULE_REVISION_247 = "2.4.7-validation-integrity-guard-2026-07-20"
RULE_REVISION_248 = "2.4.8-stage-integrity-guard-2026-07-21"
RULE_REVISION_249 = "2.4.9-gate-state-contract-2026-07-21"
...
def valid_data_245(*, approved_script_path: str = "approved/sample.approved_script.txt") -> dict:
    data = valid_data_244(approved_script_path=approved_script_path)
    data["metadata"]["version"] = "2.4.5"
    data["metadata"]["rule_revision"] = RULE_REVISION_245
    data["continuity_logs"][0]["spatial_axes"] = [
        {
            "axis_id": "AX01",
            "axis_type": "movement",
            "endpoint_a": "门口",
    ...
    dialogue_fact = data["beats"][2]["facts"][0]
    dialogue_fact.update({"speaker": "A", "addressees": [], "delivery": "onscreen"})
    data["performance_chains"] = []
    data["directorial_constraints"] = []
    shot1, shot2 = data["shots"]
    shot1.update(
        {
            "camera_geometry": {
                "axis_id": "AX01",
                "axis_side": "side_a",
                "camera_position": "桌边朝门口一侧",
                "camera_facing": "toward_end_a",
                "angle_delta_from_previous": "scene_start",
                "primary_subjects": ["A"],
                "pov_character": None,
                "screen_directions": [
                    {"entity": "A", "kind": "facing", "direction": "screen_right"}
                ],
            },
            "performance_cues": [
                {
                    "character": "A",
                    "channel": "posture_hand",
                    "description": "手扶门框",
    ...
    shot2.update(
        {
            "camera_geometry": {
                "axis_id": "AX01",
                "axis_side": "side_a",
                "camera_position": "桌侧平行位",
                "camera_facing": "along_a_to_b",
                "angle_delta_from_previous": "substantial",
                "primary_subjects": ["A"],
                "pov_character": None,
                "screen_directions": [
                    {"entity": "A", "kind": "facing", "direction": "screen_right"},
                    {"entity": "A", "kind": "movement", "direction": "screen_right"},
                ],
            },
            "performance_cues": [
                {
...
def valid_data_246(*, approved_script_path: str = "approved/sample.approved_script.txt") -> dict:
    data = valid_data_245(approved_script_path=approved_script_path)
    data["metadata"]["version"] = "2.4.6"
    data["metadata"]["rule_revision"] = RULE_REVISION_246
    return data


def valid_data_247(*, approved_script_path: str = "approved/sample.approved_script.txt") -> dict:
    data = valid_data_246(approved_script_path=approved_script_path)
    data["metadata"]["version"] = "2.4.7"
    data["metadata"]["rule_revision"] = RULE_REVISION_247
    return data


def valid_data_248(*, approved_script_path: str = "approved/sample.approved_script.txt") -> dict:
    data = valid_data_247(approved_script_path=approved_script_path)
    data["metadata"]["version"] = "2.4.8"
    data["metadata"]["rule_revision"] = RULE_REVISION_248
    return data


def valid_data_249(*, approved_script_path: str = "approved/sample.approved_script.txt") -> dict:
    data = valid_data_248(approved_script_path=approved_script_path)
    data["metadata"]["version"] = "2.4.9"
    data["metadata"]["rule_revision"] = RULE_REVISION_249
    data["metadata"]["reference_status"]["gate-workflow"] = "loaded"
    data["metadata"]["reference_proof"]["gate-workflow"] = "# Gate 状态机与回流合同"
    return data
...
def prepare_current_fixture(
    ...
    *,
    ...
    if result.errors:
        raise AssertionError(f"invalid 2.4.9 fixture: {result.errors}")
...
class DeliveryContractTests(unittest.TestCase):
    def test_build_writes_7_column_markdown_and_excel(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data, data_path, md_path, xlsx_path, report_path = prepare_current_fixture(root)
...
class ContractIntegrity243Tests(unittest.TestCase):
    def test_245_version_profile_is_fail_closed_and_build_only(self) -> None:
        self.assertEqual("2.4.9", delivery.VERSION)
        self.assertEqual(RULE_REVISION_249, delivery.RULE_REVISION)
        ...
        self.assertTrue(delivery.VERSION_PROFILES["2.4.5"]["contract_integrity"])
        self.assertTrue(delivery.VERSION_PROFILES["2.4.6"]["contract_integrity"])
        self.assertTrue(delivery.VERSION_PROFILES["2.4.7"]["contract_integrity"])
        self.assertTrue(delivery.VERSION_PROFILES["2.4.8"]["contract_integrity"])
        self.assertTrue(delivery.VERSION_PROFILES["2.4.9"]["gate_workflow_guard"])
        ...
        self.assertTrue(delivery.VERSION_PROFILES["2.4.5"]["director_integrity_guard"])
        self.assertFalse(delivery.VERSION_PROFILES["2.4.5"]["dialogue_partition_guard"])
        self.assertTrue(delivery.VERSION_PROFILES["2.4.6"]["dialogue_partition_guard"])
        self.assertFalse(delivery.VERSION_PROFILES["2.4.6"]["validation_integrity_guard"])
        self.assertTrue(delivery.VERSION_PROFILES["2.4.7"]["validation_integrity_guard"])
        self.assertFalse(delivery.VERSION_PROFILES["2.4.7"]["stage_integrity_guard"])
        self.assertTrue(delivery.VERSION_PROFILES["2.4.8"]["stage_integrity_guard"])
    ...
    def test_245_version_and_rule_revision_markers_are_consistent_everywhere(self) -> None:
        skill_root = SCRIPT_DIR.parent
        expected_version = "2.4.9"
        expected_revision = RULE_REVISION_249
        ...
        self.assertIn("Legacy compatibility wrapper", wrapper)
        self.assertIn("formal entry is storyboard_delivery.py", wrapper)
        ...
        contract_files = [
            ...
            skill_root / "references" / "gate-workflow.md",
        ]
    ...
    def test_243_real_approved_script_path_and_content_pass(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data, data_path, _, _, _ = prepare_current_fixture(root)
            result = delivery.validate_data(
                data,
                strict_status=True,
                data_path=data_path,
    ...
    def test_245_dialogue_labels_accept_performance_directions_and_adjacent_vo(self) -> None:
        source = (
            "林晓杰（虚弱地笑）：你来了。比我想的……晚了一点。\n"
            "林晓彤(眼眶微红)：晓杰。\n"
            "雾VO（怒吼）：你们想干什么！\n"
            "沈夜（VO）（压低声音）：别怕。"
        )
        ...
        self.assertEqual(
            ...
            delivery.dialogue_candidates(source),
        )
        self.assertEqual(
            [
                ("林晓杰", "你来了。比我想的……晚了一点。", False),
                ("林晓彤", "晓杰。", False),
                ("雾", "你们想干什么！", True),
                ("沈夜", "别怕。", True),
            ],
            delivery.dialogue_records(source),
        )

    def test_245_dialogue_facts_accept_exact_ordered_partition(self) -> None:
        source = "林晓杰：可你知道吗……最痛苦的不是恨。是每次在走廊里看见你，我心里那个“恶念”让我扑上去杀了你……可另一个声音，却在喊“姐姐”。"
        facts = [
            {"fact_id": "B005-F03", "type": "dialogue", "text": "可你知道吗……最痛苦的不是恨。", "speaker": "林晓杰", "delivery": "onscreen"},
            {"fact_id": "B005-F06", "type": "dialogue", "text": "是每次在走廊里看见你，我心里那个“恶念”让我扑上去杀了你……可另一个声音，却在喊“姐姐”。", "speaker": "林晓杰", "delivery": "onscreen"},
        ]

        self.assertEqual(
            {"B005-F03", "B005-F06"},
            delivery.dialogue_partition_fact_ids(source, facts),
        )
        result = delivery.ValidationResult()
        delivery.validate_fact_text_preservation(
            {"metadata": {"version": "2.4.6"}, "beats": [{"source_text": source, "facts": facts}]},
            result,
        )
        self.assertFalse(result.errors, result.errors)

        legacy_result = delivery.ValidationResult()
        delivery.validate_fact_text_preservation(
            {"metadata": {"version": "2.4.5"}, "beats": [{"source_text": source, "facts": facts}]},
            legacy_result,
        )
        self.assertTrue(legacy_result.errors, legacy_result.errors)

    def test_245_dialogue_fact_partition_rejects_loss_overlap_reorder_and_cross_speaker(self) -> None:
        cases = {
            "omission": (
                "甲：第一句。第二句。",
                [("B001-F01", "第一句。", "甲", "onscreen"), ("B001-F02", "第二句", "甲", "onscreen")],
            ),
            "overlap": (
                "甲：第一句。第二句。",
                [("B001-F01", "第一句。", "甲", "onscreen"), ("B001-F02", "一句。第二句。", "甲", "onscreen")],
            ),
            "reorder": (
                "甲：第一句。第二句。",
                [("B001-F01", "第二句。", "甲", "onscreen"), ("B001-F02", "第一句。", "甲", "onscreen")],
            ),
            "cross speaker": (
                "甲：第一句。\n乙：第二句。",
                [("B001-F01", "第一句。第", "甲", "onscreen"), ("B001-F02", "二句。", "甲", "onscreen")],
            ),
            "vo mismatch": (
                "甲（VO）：第一句。第二句。",
                [("B001-F01", "第一句。", "甲", "onscreen"), ("B001-F02", "第二句。", "甲", "onscreen")],
        ...
        for label, (source, parts) in cases.items():
            with self.subTest(label=label):
                facts = [
                    {"fact_id": fact_id, "type": "dialogue", "text": text, "speaker": speaker, "delivery": delivery_type}
                    for fact_id, text, speaker, delivery_type in parts
                ]
                self.assertEqual(set(), delivery.dialogue_partition_fact_ids(source, facts))
                result = delivery.ValidationResult()
                delivery.validate_fact_text_preservation(
                    {"metadata": {"version": "2.4.6"}, "beats": [{"source_text": source, "facts": facts}]},
                    result,
    ...
    def test_247_dialogue_partition_rejects_internal_quote_prefix_suffix_and_interposed_fact(self) -> None:
        source = "林晓杰：最痛苦的不是恨，是心里那个“恶念”在喊“姐姐”。"
        cases = {
            "internal quote only": [
                {"fact_id": "B001-F01", "type": "dialogue", "text": "恶念", "speaker": "林晓杰", "delivery": "onscreen"}
            ],
            "missing prefix": [
                {"fact_id": "B001-F01", "type": "dialogue", "text": "是心里那个“恶念”在喊“姐姐”。", "speaker": "林晓杰", "delivery": "onscreen"}
            ],
            "missing suffix": [
                {"fact_id": "B001-F01", "type": "dialogue", "text": "最痛苦的不是恨，", "speaker": "林晓杰", "delivery": "onscreen"}
            ],
            "interposed action": [
                {"fact_id": "B001-F01", "type": "dialogue", "text": "最痛苦的不是恨，", "speaker": "林晓杰", "delivery": "onscreen"},
                {"fact_id": "B001-F02", "type": "action", "text": "林晓杰停顿。"},
                {"fact_id": "B001-F03", "type": "dialogue", "text": "是心里那个“恶念”在喊“姐姐”。", "speaker": "林晓杰", "delivery": "onscreen"},
            ],
        }
                ...
                delivery.validate_fact_text_preservation(
                    {
                        ...
                        "beats": [{"beat_id": "B001", "source_text": source, "facts": facts}],
                    },
                    result,
                )
                self.assertTrue(any("全部原文对白" in item for item in result.errors), result.errors)

    def test_247_dialogue_partition_accepts_spaces_long_labels_and_is_nonrecursive(self) -> None:
        long_speaker = "Captain-Alexander-The-Third"
        long_direction = "very quietly while holding back a long and visibly difficult breath"
        source = f"{long_speaker}（{long_direction}）：Hello world."
        facts = [
            {"fact_id": "B001-F01", "type": "dialogue", "text": "Hello ", "speaker": long_speaker, "delivery": "onscreen"},
            {"fact_id": "B001-F02", "type": "dialogue", "text": "world.", "speaker": long_speaker, "delivery": "onscreen"},
        ]
        self.assertEqual(
            {"B001-F01", "B001-F02"},
            delivery.dialogue_partition_fact_ids(source, facts, known_speakers={long_speaker}),
        )
        ...
        for index in range(1100):
            speaker = f"角色-{index}"
            source_lines.append(f"{speaker}：line {index}.")
            many_facts.append(
                {
                    "fact_id": f"B001-F{index + 1:04d}",
                    "type": "dialogue",
                    "text": f"line {index}.",
                    "speaker": speaker,
                    "delivery": "onscreen",
        ...
        approved = delivery.dialogue_partition_fact_ids(
            "\n".join(source_lines),
            many_facts,
            known_speakers={fact["speaker"] for fact in many_facts},
        )
        self.assertEqual(1100, len(approved))

    def test_247_dialogue_delivery_is_exact_and_historical_parsers_are_frozen(self) -> None:
        source = "雾（VO）：受死吧。"
        mismatch = [
            {"fact_id": "B001-F01", "type": "dialogue", "text": "受死吧。", "speaker": "雾", "delivery": "offscreen"}
        ]
        self.assertEqual(
            set(),
            delivery.dialogue_partition_fact_ids(source, mismatch, known_speakers={"雾"}),
        )

        internal_quote_source = "林晓杰：我心里那个“恶念”却在喊“姐姐”。"
        self.assertIn(
            "恶念",
            delivery.dialogue_candidates_for_data(
                {"metadata": {"version": "2.4.6"}}, internal_quote_source
            ),
        )
        self.assertIn(
            "恶念",
            delivery.dialogue_candidates_for_data(
                {"metadata": {"version": "2.4.5"}}, internal_quote_source
            ),
        )
        self.assertNotIn(
            "恶念",
            delivery.dialogue_candidates_for_data(
                {"metadata": {"version": "2.4.7"}}, internal_quote_source
            ),
        )

    def test_247_revision_log_requires_exact_continuous_canonical_entries(self) -> None:
        valid = valid_data_247()
        valid["metadata"]["revision_log"] = [
            {"round": 1, "trigger": "Gate A 驳回", "summary": "修正事实映射"},
            {"round": 2, "trigger": "校验失败", "summary": "修正对白分区"},
        ]
        result = delivery.ValidationResult()
        delivery.validate_metadata(valid, result, strict_status=False, final_signoff=False)
        self.assertFalse(any("revision_log" in item for item in result.errors), result.errors)

        invalid_entries = [
            [{"round": 2, "trigger": "跳轮", "summary": "非法"}],
            [{"round": 1, "trigger": "  带空白", "summary": "非法"}],
            [{"round": 1, "trigger": "合法", "summary": "非法", "junk": True}],
            ["free-form junk"],
        ]
        for revision_log in invalid_entries:
            with self.subTest(revision_log=revision_log):
                data = valid_data_247()
                data["metadata"]["revision_log"] = revision_log
                invalid_result = delivery.ValidationResult()
                delivery.validate_metadata(data, invalid_result, strict_status=False, final_signoff=False)
                self.assertTrue(
                    any("revision_log" in item for item in invalid_result.errors),
                    invalid_result.errors,
                )

    def test_247_node_wrapper_requires_explicit_python_runtime(self) -> None:
        ...
        self.assertIn("Missing --python", process.stderr)
    ...
    def test_243_latest_review_rejection_duplicate_round_and_stale_hash_fail(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            base, data_path, _, _, _ = prepare_current_fixture(root)
    ...
    def test_243_full_build_gate_c_validate_and_node_wrapper_flow(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data, data_path, markdown_path, excel_path, report_path = prepare_current_fixture(root)
            delivery.write_json(data_path, data)
            common = [
                "--data",
                str(data_path),
    ...
    def test_243_build_requires_report_without_mutating_input(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data, data_path, markdown_path, excel_path, _ = prepare_current_fixture(root)
    ...
    def test_245_pre_signoff_warn_can_be_kept_then_finally_signed(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            approved = root / "approved" / "sample.approved_script.txt"
            approved.parent.mkdir(parents=True, exist_ok=True)
...
class DirectorShotIntegrity245Tests(unittest.TestCase):
    def validate_245(
        self,
        data: dict,
        root: Path,
        *,
        ...
        data["human_reviews"] = []
        append_review(data, "GATE_A")
        append_review(data, "GATE_B", approved_items=approved_items or [])
        delivery.derive_prompts(data)
        return delivery.validate_data(
            data,
            strict_status=False,
    ...
    def test_same_axis_minor_change_fails_even_when_shot_size_changes(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["shots"][1]["camera_geometry"]["angle_delta_from_previous"] = "minor"
            data["shots"][1]["camera_main_image"] = data["shots"][1]["camera_main_image"].replace(
                "中景", "特写", 1
            )
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("等价30度" in item for item in result.errors), result.errors)

    def test_not_comparable_cannot_hide_same_primary_subject(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["shots"][1]["camera_geometry"]["angle_delta_from_previous"] = "not_comparable"
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("不得用 not_comparable" in item for item in result.errors), result.errors)

            data["shots"][1]["camera_geometry"]["primary_subjects"] = ["桌面"]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("primary_subjects 必须映射" in item for item in result.errors), result.errors)
            self.assertTrue(any("不得用 not_comparable" in item for item in result.errors), result.errors)

    def test_intentional_jump_cut_requires_marker_and_exact_gate_b_token(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["shots"][1]["camera_geometry"]["angle_delta_from_previous"] = "minor"
            data["shots"][1]["notes"] += " [跳切说明] 同一动作内部的有意断裂。"
            result = self.validate_245(
                data, Path(root_name), approved_items=["shot:2:jump-cut"]
            )
            self.assertFalse(any("等价30度" in item for item in result.errors), result.errors)
            self.assertTrue(any("跳切" in item for item in result.warnings), result.warnings)

    def test_direct_axis_cross_and_unmotivated_screen_reversal_fail(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            crossed = valid_data_245()
            crossed["shots"][1]["camera_geometry"].update(
                {"axis_side": "side_b", "angle_delta_from_previous": "axis_cross"}
            )
            result = self.validate_245(crossed, root)
            self.assertTrue(any("直接切到另一侧" in item for item in result.errors), result.errors)

            reversed_data = valid_data_245()
            reversed_data["shots"][1]["camera_geometry"]["screen_directions"][0][
                "direction"
            ] = "screen_left"
            reversed_data["shots"][1]["continuity_updates"] = []
            reversed_data["shots"][1]["camera_main_image"] = reversed_data["shots"][1][
                "camera_main_image"
            ].replace("【站位位移】", "")
            result = self.validate_245(reversed_data, root)
            self.assertTrue(any("屏幕方向反转" in item for item in result.errors), result.errors)

    def test_subjective_shot_requires_pov_character(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["shots"][0]["camera_main_image"] = data["shots"][0]["camera_main_image"].replace(
                "平视", "主观视角", 1
            ...
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("主观镜必须填写" in item for item in result.errors), result.errors)

    def test_same_character_performance_chain_defaults_to_one_shot(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["performance_chains"] = [
                {
                    ...
                    "steps": [
                        ...
                        {"role": "dialogue", "fact_ids": ["B003-F01"]},
                    ],
                }
            ]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("动作→反应→台词链未落在同一镜" in item for item in result.errors), result.errors)

    def test_omitting_performance_chain_cannot_bypass_same_character_default(self) -> None:
        minimal = {
            "metadata": {"version": "2.4.5"},
            "continuity_logs": [
                {"scene_id": "S01", "characters": [{"name": "A"}]}
            ],
            "beats": [
                {
                    "beat_id": "B001",
                    ...
                    "facts": [
                        ...
                        {"fact_id": "B001-F02", "type": "space", "text": "走廊仍然安静。"},
                        {
                            "fact_id": "B001-F03",
                            "type": "dialogue",
                            "text": "到了。",
            ...
            "shots": [
                {"shot_no": 1, "scene_id": "S01", "covered_fact_ids": ["B001-F01"], "chain_breaks": []},
                {"shot_no": 2, "scene_id": "S01", "covered_fact_ids": ["B001-F03"], "chain_breaks": []},
            ],
            "performance_chains": [],
            "directorial_constraints": [],
        }
        result = delivery.ValidationResult()
        delivery.validate_performance_chains_and_constraints(minimal, result)
        self.assertTrue(any("被拆到不同镜头" in item for item in result.errors), result.errors)

    def test_action_and_emotion_performers_are_structured(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["beats"][1]["facts"][0].update(
                {"type": "emotion", "text": "A笑了一下。"}
            )
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("emotion performers 必须是数组" in item for item in result.errors), result.errors)

            data["beats"][1]["facts"][0]["performers"] = []
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("emotion performers 不能为空" in item for item in result.errors), result.errors)
            self.assertTrue(any("performers 未登记" in item for item in result.errors), result.errors)

            data["beats"][1]["facts"][0].update({"type": "action", "performers": []})
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("action performers 不能为空" in item for item in result.errors), result.errors)

    def test_performer_mapping_must_match_shot_and_performance_cue_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["continuity_logs"][0]["characters"].append(
                {"name": "B", "position": "桌后", "facing": "A"}
            )
            data["beats"][1]["facts"][0].update(
                {"type": "emotion", "text": "笑意浮现。", "performers": ["B"]}
            )
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("performers 未映射到镜头" in item for item in result.errors), result.errors)
            self.assertTrue(any("必须包含在证据事实" in item for item in result.errors), result.errors)

            actorless = valid_data_245()
            actorless["beats"][1]["facts"][0].update(
                {"type": "action", "text": "笑意浮现。", "performers": []}
            )
            result = self.validate_245(actorless, Path(root_name))
            self.assertTrue(any("action performers 不能为空" in item for item in result.errors), result.errors)
            self.assertTrue(any("必须包含在证据事实" in item for item in result.errors), result.errors)

    def test_performance_cue_cannot_launder_character_action_through_space_fact(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["beats"][1]["facts"][0].update(
                {"type": "action", "text": "笑意浮现。", "performers": []}
            )
            data["shots"][1]["performance_cues"][0]["evidence_fact_ids"] = ["B001-F01"]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("action performers 不能为空" in item for item in result.errors), result.errors)

            data["beats"][0]["facts"][0]["type"] = "space"
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("只能引用 action / emotion / dialogue / position" in item for item in result.errors), result.errors)

    def test_adjacent_identical_source_span_requires_structured_reuse_exception(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["shots"][1].pop("source_spans")
            data["shots"][1]["source_span"] = copy.deepcopy(data["shots"][0]["source_span"])
            data["shots"][1]["source_paragraph"] = data["shots"][0]["source_paragraph"]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("完全相同 source_span" in item for item in result.errors), result.errors)

    def test_source_span_hash_presence_cannot_hide_duplicate_range(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["shots"][1].pop("source_spans")
            data["shots"][1]["source_span"] = copy.deepcopy(data["shots"][0]["source_span"])
            data["shots"][1]["source_span"]["text_hash"] = delivery.normalized_script_text_hash(
                data["script_lock"]["locked_text"][0:6]
            )
            data["shots"][1]["source_paragraph"] = data["shots"][0]["source_paragraph"]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("完全相同 source_span" in item for item in result.errors), result.errors)

    def test_overlapping_step_pairs_do_not_satisfy_whole_performance_chain(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["shots"][0]["beat_ids"].append("B002")
            data["shots"][0]["covered_fact_ids"].append("B002-F01")
            data["performance_chains"] = [
                {
                    "chain_id": "PC001",
                    "scene_id": "S01",
                    ...
                    "steps": [
                        {"role": "action", "fact_ids": ["B001-F01"]},
                        {"role": "action", "fact_ids": ["B002-F01"]},
                        {"role": "dialogue", "fact_ids": ["B003-F01"]},
                    ],
                }
            ]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("整链合并" in item for item in result.errors), result.errors)

    def test_structured_source_reuse_and_chain_break_can_only_be_human_exceptions(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            reused = valid_data_245()
            reused["shots"][1].pop("source_spans")
            reused["shots"][1]["source_span"] = copy.deepcopy(reused["shots"][0]["source_span"])
            reused["shots"][1]["source_paragraph"] = reused["shots"][0]["source_paragraph"]
            reused["shots"][1]["source_reuse"] = {
                "from_shot_no": 1,
                "reason": "source_line_indivisible",
            }
            reused["shots"][1]["notes"] += " [原文复用] 单句内存在不可合并动作。"
            result = self.validate_245(
                reused, root, approved_items=["shot:2:source-reuse"]
            )
            self.assertFalse(any("默认视为重复拆镜" in item for item in result.errors), result.errors)
            self.assertTrue(any("复用同一原文跨度" in item for item in result.warnings), result.warnings)

            chained = valid_data_245()
            chained["performance_chains"] = [
                {
                    "chain_id": "PC001",
                    "scene_id": "S01",
                    "character": "A",
            ...
            chained["shots"][1]["chain_breaks"] = [
                {"chain_id": "PC001", "after_step": 1, "reason": "intentional_editorial_emphasis"}
            ]
            chained["shots"][1]["notes"] += " [动作链拆分] 经人工确认的编辑强调。"
            result = self.validate_245(
                chained, root, approved_items=["shot:2:performance-chain-break"]
            )
            self.assertFalse(any("未落在同一镜" in item for item in result.errors), result.errors)
            self.assertTrue(any("动作链拆分" in item for item in result.warnings), result.warnings)

    def test_performance_action_and_environment_evidence_are_enforced(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            missing_cue = valid_data_245()
            missing_cue["shots"][0]["performance_cues"] = []
            result = self.validate_245(missing_cue, root)
            self.assertTrue(any("performance_cues 必须为 1-3 项" in item for item in result.errors), result.errors)

            missing_progression = valid_data_245()
            missing_progression["shots"][1]["action_progressions"] = []
            result = self.validate_245(missing_progression, root)
            self.assertTrue(any("必须填写 action_progressions" in item for item in result.errors), result.errors)

            missing_environment = valid_data_245()
            missing_environment["shots"][0]["camera_main_image"] += "\n白光漫过门框。"
            result = self.validate_245(missing_environment, root)
            self.assertTrue(any("必须填写 environment_cues" in item for item in result.errors), result.errors)

    def test_dialogue_role_mapping_is_structured(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["beats"][2]["facts"][0]["speaker"] = "B"
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("speaker 必须精确匹配" in item for item in result.errors), result.errors)

    def test_voiceover_cannot_be_a_pictureless_dialogue_only_shot(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            dialogue = data["beats"][2]["facts"][0]
            dialogue["delivery"] = "vo"
            data["shots"][1]["offscreen_characters"] = ["A"]
            data["shots"][1]["visible_characters"] = []
            data["shots"][1]["performance_cues"] = []
            data["shots"][1]["covered_fact_ids"] = ["B003-F01"]
            data["shots"][1]["beat_ids"] = ["B003"]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("无画面事实支撑" in item for item in result.errors), result.errors)

    def test_same_shot_directorial_constraint_survives_regeneration(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["directorial_constraints"] = [
                {
                    ...
                    "type": "same_shot",
                    "freeze_gate": "GATE_A",
                    "origin": {"gate": "GATE_A", "round": 1},
                    "fact_ids": ["B001-F01", "B003-F01"],
                }
            ]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("要求的事实未落在同一镜" in item for item in result.errors), result.errors)

    def test_same_beat_constraint_forces_gate_a_beat_merge(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["directorial_constraints"] = [
                {
                    ...
                    "freeze_gate": "GATE_A",
                    "origin": {"gate": "GATE_A", "round": 1},
                    "fact_ids": ["B002-F01", "B003-F01"],
                }
            ]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("不属于同一 Beat" in item for item in result.errors), result.errors)

    def test_forbid_literal_and_direct_handoff_constraints_are_executable(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            data = valid_data_245()
            data["directorial_constraints"] = [
                {
                    ...
                    "type": "forbid_literal",
                    "freeze_gate": "GATE_B",
                    "origin": {"gate": "GATE_B", "round": 1},
                    "literal": "门框",
                    "scene_ids": ["S01"],
                }
            ]
            result = self.validate_245(data, Path(root_name))
            self.assertTrue(any("禁止字面元素" in item for item in result.errors), result.errors)
            ...
            "beats": [
                {
                    "beat_id": "B001",
                    "scene_id": "S01",
                    "facts": [{"fact_id": "B001-F01", "type": "space"}],
                }
            ],
            "shots": [
                {
            ...
            "directorial_constraints": [
                {
                    ...
                    "freeze_gate": "GATE_A",
                    "origin": {"gate": "GATE_A", "round": 1},
                    "from_fact_id": "B001-F01",
                    "to_scene_id": "S02",
                    "forbidden_literal": "白光",
                }
            ],
        }
        result = delivery.ValidationResult()
        delivery.validate_performance_chains_and_constraints(minimal, result)
        self.assertTrue(any("目标场首镜再次出现" in item for item in result.errors), result.errors)

    def test_rhythm_warnings_require_human_resolution_in_245(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data = valid_data_245()
            result = self.validate_245(data, root)
            rhythm = [message for message in result.warnings if "节奏预警" in message]
            self.assertTrue(rhythm, result.warnings)
            resolve_warnings(data, result, resolved_by="auto_whitelist")
            result = delivery.validate_data(
                data,
                strict_status=False,
                data_path=root / "sample.shot_data.json",
            ...
            self.assertTrue(any("不是白名单 WARN" in item for item in result.errors), result.errors)

    def test_244_legacy_data_does_not_require_245_director_fields(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data = valid_data_244()
            approved = root / "approved" / "sample.approved_script.txt"
            ...
            self.assertFalse(
                any(
                    ...
                    for token in ("camera_geometry", "performance_chains", "directorial_constraints")
                ),
                result.errors,
            )

    def test_gate_b_review_package_is_deterministic_and_read_only(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data, data_path, _, _, _ = prepare_current_fixture(root)
            delivery.write_json(data_path, data)
            before = data_path.read_bytes()
            output = root / "sample.gate-b-review.md"
            rc = delivery.main(
                [
                    "review-package",
                    "--workspace-root",
            ...
            for heading in (
                ...
                self.assertIn(heading, package)
            second = root / "sample.gate-b-review-2.md"
            self.assertEqual(
                0,
                delivery.main(
                    [
            ...
            self.assertEqual(package, second.read_text(encoding="utf-8"))
            self.assertEqual(before, data_path.read_bytes())

    def test_247_gate_b_review_package_fails_closed_without_gate_a_or_on_audit_error(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            approved = root / "approved" / "sample.approved_script.txt"
            approved.parent.mkdir(parents=True, exist_ok=True)

            missing_gate_a = valid_data_249()
            approved.write_text(missing_gate_a["script_lock"]["locked_text"], encoding="utf-8")
            missing_path = root / "missing-gate-a.shot_data.json"
            missing_output = root / "missing-gate-a.gate-b-review.md"
            delivery.write_json(missing_path, missing_gate_a)
            self.assertEqual(
                1,
                delivery.main(
                    [
            ...
            audit_failure = valid_data_249()
            append_review(audit_failure, "GATE_A")
            audit_failure["metadata"]["revision_log"] = ["junk"]
            audit_path = root / "audit-failure.shot_data.json"
            audit_output = root / "audit-failure.gate-b-review.md"
            delivery.write_json(audit_path, audit_failure)
            self.assertEqual(
                1,
                delivery.main(
                    [
    ...
    def test_247_gate_b_review_package_accepts_promptless_director_table_only(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data = valid_data_249()
            approved = root / "approved" / "sample.approved_script.txt"
            ...
            data_path = root / "promptless.shot_data.json"
            output = root / "promptless.gate-b-review.md"
            delivery.write_json(data_path, data)
            before = data_path.read_bytes()

            full_audit = delivery.validate_data(
                data,
                strict_status=False,
                data_path=data_path,
                workspace_root=root,
            )
            self.assertTrue(any("Prompt" in item or "prompt" in item for item in full_audit.errors))
    ...
    def test_247_gate_b_review_package_accepts_pending_performance_chain_break_only(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data = valid_data_249()
            for beat in data["beats"][:2]:
                beat["facts"][0]["type"] = "action"
                beat["facts"][0]["performers"] = ["A"]
            data["performance_chains"] = [
                {
                    "chain_id": "PC001",
                    "scene_id": "S01",
            ...
            data["shots"][1]["chain_breaks"] = [
                {
                    "chain_id": "PC001",
                    "after_step": 1,
                    "reason": "intentional_editorial_emphasis",
                }
            ]
            data["shots"][1]["notes"] += " [动作链拆分] 待 Gate B 复核。"
            approved = root / "approved" / "sample.approved_script.txt"
            approved.parent.mkdir(parents=True, exist_ok=True)
            approved.write_text(data["script_lock"]["locked_text"], encoding="utf-8")
            append_review(data, "GATE_A")

            data_path = root / "pending-chain-break.shot_data.json"
            output = root / "pending-chain-break.gate-b-review.md"
            delivery.write_json(data_path, data)
            before = data_path.read_bytes()
            ...
            self.assertTrue(any("PC001" in item for item in full_audit.errors), full_audit.errors)
            ...
            package = output.read_text(encoding="utf-8")
            self.assertIn("PC001", package)
            self.assertIn("待 Gate B 精确批准", package)
            self.assertEqual(before, data_path.read_bytes())

            append_review(
                data,
                "GATE_B",
                approved_items=["shot:2:performance-chain-break"],
            )
            approved_audit = delivery.validate_data(
                data,
                strict_status=False,
                data_path=data_path,
                workspace_root=root,
            )
            self.assertFalse(any("PC001" in item for item in approved_audit.errors), approved_audit.errors)

    def test_247_gate_b_review_package_validates_only_completed_batch_prefix(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data = valid_data_249()
            data["batch_plan"] = {
                "batches": [
                    {"batch_id": "BT01", "scene_ids": ["S01"], "expected_shot_count": 2, "depends_on": []},
                    {"batch_id": "BT02", "scene_ids": ["S02"], "expected_shot_count": 2, "depends_on": ["BT01"]},
                ]
            }
            approved = root / "approved" / "sample.approved_script.txt"
            approved.parent.mkdir(parents=True, exist_ok=True)
            ...
            append_review(data, "GATE_A", batch_id="BT01")
            delivery.derive_prompts(data)
            data_path = root / "batch-prefix.shot_data.json"
            output = root / "batch-prefix.gate-b-review.md"
            delivery.write_json(data_path, data)
            ...
            self.assertTrue(output.exists())
            self.assertIn("batch_id: BT01", output.read_text(encoding="utf-8"))


class StageIntegrity248Tests(unittest.TestCase):
    def test_gate_a_review_hash_refuses_missing_position_anchor_then_accepts_split_fact(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data = valid_data_249()
            log = data["continuity_logs"][0]
            log["first_shot_anchor_type"] = "both"
            log["characters"].append({"name": "B", "position": "桌边", "facing": "门口"})
            for beat in data["beats"][:2]:
                beat["facts"][0]["type"] = "action"
                beat["facts"][0]["performers"] = ["A"]
            approved = root / "approved" / "sample.approved_script.txt"
            approved.parent.mkdir(parents=True, exist_ok=True)
            approved.write_text(data["script_lock"]["locked_text"], encoding="utf-8")
            data_path = root / "gate-a.shot_data.json"
            delivery.write_json(data_path, data)
            command = [
                "review-hash",
                "--workspace-root",
            ...
            with mock.patch("builtins.print") as printer:
                self.assertEqual(1, delivery.main(command))
            printed = "\n".join(str(call.args[0]) for call in printer.call_args_list if call.args)
            self.assertIn("不存在 position", printed)

            source_fact = data["beats"][0]["facts"][0]
            position_fact = copy.deepcopy(source_fact)
            position_fact.update(
                {
                    "fact_id": "B001-F02",
                    "type": "position",
                    "text": "A站在门口。",
                    "cut_group": "B001-F02",
                    "cut_category": "space",
                    "cut_moment_id": "B001-position-moment",
                }
            )
            position_fact.pop("performers", None)
            data["beats"][0]["facts"].append(position_fact)
            space_fact = copy.deepcopy(position_fact)
            space_fact.update(
                {
                    "fact_id": "B001-F03",
                    "type": "space",
                    "text": "A站在门口。",
                    "cut_group": "B001-F03",
                    "cut_moment_id": "B001-space-moment",
                }
            )
            data["beats"][0]["facts"].append(space_fact)
            delivery.write_json(data_path, data)
            with mock.patch("builtins.print") as printer:
                self.assertEqual(0, delivery.main(command))
            payload = json.loads(printer.call_args.args[0])
            self.assertRegex(payload["reviewed_hash"], r"^[0-9a-f]{64}$")

    def test_auto_performance_inference_stops_at_beat_and_nonperformance_boundaries(self) -> None:
        data = valid_data_249()
        first = data["beats"][0]["facts"][0]
        second = data["beats"][1]["facts"][0]
        first.update({"type": "action", "performers": ["A"]})
        second.update({"type": "emotion", "performers": ["A"]})
        result = delivery.ValidationResult()
        delivery.validate_performance_chains_and_constraints(data, result)
        self.assertFalse(any("检测到 A 的连续" in item for item in result.errors), result.errors)

        within_beat = copy.deepcopy(data)
        later = copy.deepcopy(second)
        later["fact_id"] = "B001-F02"
        within_beat["beats"][0]["facts"].append(later)
        result = delivery.ValidationResult()
        delivery.validate_performance_chains_and_constraints(within_beat, result)
        self.assertTrue(any("检测到 A 的连续" in item for item in result.errors), result.errors)

        separated = copy.deepcopy(within_beat)
        separator = copy.deepcopy(first)
        separator.update({"fact_id": "B001-F03", "type": "position"})
        separator.pop("performers", None)
        separated["beats"][0]["facts"] = [first, separator, later]
        result = delivery.ValidationResult()
        delivery.validate_performance_chains_and_constraints(separated, result)
        self.assertFalse(any("检测到 A 的连续" in item for item in result.errors), result.errors)

    def test_performance_chain_role_and_order_are_fail_closed(self) -> None:
        data = valid_data_249()
        data["performance_chains"] = [
            {
                "chain_id": "PC001",
        ...
        result = delivery.ValidationResult()
        delivery.validate_performance_chains_and_constraints(
            data,
            result,
            require_shot_resolution=False,
        )
        self.assertTrue(any("只能引用 action / emotion" in item for item in result.errors), result.errors)
        self.assertTrue(any("叙事顺序严格递增" in item for item in result.errors), result.errors)

    def test_gate_b_prefix_excludes_later_batch_chains_and_direct_handoffs(self) -> None:
        data = valid_data_249()
        data["batch_plan"] = {
            "batches": [
                {"batch_id": "BT01", "scene_ids": ["S01"], "expected_shot_count": 2, "depends_on": []},
                {"batch_id": "BT02", "scene_ids": ["S02"], "expected_shot_count": 1, "depends_on": ["BT01"]},
            ]
        }
        data["performance_chains"] = [
            {"chain_id": "PC999", "scene_id": "S02", "character": "X", "steps": []}
        ]
        data["directorial_constraints"] = [
            {
                "constraint_id": "DC999",
                ...
                "freeze_gate": "GATE_B",
                "origin": {"gate": "GATE_B", "round": 1},
                "from_fact_id": "B999-F01",
                "to_scene_id": "S02",
                "forbidden_literal": "later",
            }
        ]
        audit = delivery.gate_b_audit_data(data, "BT01")
        self.assertEqual([], audit["performance_chains"])
        self.assertEqual([], audit["directorial_constraints"])

    def test_group_performance_cues_expand_only_to_obligated_visible_performers(self) -> None:
        data = valid_data_249()
        log = data["continuity_logs"][0]
        log["characters"] = [
            {"name": name, "position": "门口", "facing": "桌边"}
            for name in ("A", "B", "C", "D")
        ]
        fact = data["beats"][0]["facts"][0]
        fact.update({"type": "action", "performers": ["A", "B", "C", "D"]})
        shot = data["shots"][0]
        shot["visible_characters"] = ["A", "B", "C", "D"]
        descriptions = ["A抬头", "B抬头", "C抬头", "D抬头"]
        shot["camera_main_image"] += "\n" + "，".join(descriptions) + "。"
        shot["performance_cues"] = [
            {
                "character": name,
                "channel": "gaze_face",
            ...
            for name, description in zip(("A", "B", "C", "D"), descriptions)
        ]
        result = delivery.ValidationResult()
        delivery.validate_shot_director_integrity(
            data,
            shot,
            log,
            delivery.fact_lookup(data),
            result,
            is_first_scene_shot=True,
        )
        self.assertFalse(any("performance_cues 必须" in item for item in result.errors), result.errors)

        too_many = copy.deepcopy(shot)
        too_many["performance_cues"].append(copy.deepcopy(too_many["performance_cues"][0]))
        result = delivery.ValidationResult()
        delivery.validate_shot_director_integrity(
            data,
            too_many,
        ...
        self.assertTrue(any("1-4" in item and "max(3" in item for item in result.errors), result.errors)

        only_one_obligated = copy.deepcopy(data)
        obligated_fact = only_one_obligated["beats"][0]["facts"][0]
        obligated_fact["performers"] = ["A"]
        obligated_shot = only_one_obligated["shots"][0]
        obligated_shot["performance_cues"] = copy.deepcopy(shot["performance_cues"])
        result = delivery.ValidationResult()
        delivery.validate_shot_director_integrity(
            only_one_obligated,
            obligated_shot,
            only_one_obligated["continuity_logs"][0],
            delivery.fact_lookup(only_one_obligated),
            result,
            is_first_scene_shot=True,
        )
        self.assertTrue(any("1-3" in item and "max(3" in item for item in result.errors), result.errors)

    def test_249_sound_source_migration_uses_sound_environment_closed_loop(self) -> None:
        def sound_case(version: str = "2.4.9") -> tuple[dict, dict, dict]:
            data = valid_data_249() if version == "2.4.9" else valid_data_248()
            log = data["continuity_logs"][0]
            log["sound_sources"] = {
                "SS01": {
                    "name": "门铃",
            ...
            data["beats"][0]["facts"].append(sound_fact)
            shot = data["shots"][0]
            shot["covered_fact_ids"].append("B001-F02")
            shot["camera_main_image"] += "\n门铃由沉默转为响起。"
            ...
            shot["action_progressions"] = []
            shot["environment_cues"] = [
                {
                    "kind": "sound",
                    "basis": "fact",
                    "description": "门铃由沉默转为响起",
                    "evidence_fact_ids": ["B001-F02"],
                }
            ]
            return data, log, shot

        data, log, shot = sound_case()
        result = delivery.ValidationResult()
        delivery.validate_shot_director_integrity(
            data, shot, log, delivery.fact_lookup(data), result, is_first_scene_shot=True
        )
        self.assertFalse(any("必须填写 action_progressions" in item for item in result.errors), result.errors)
        self.assertFalse(any("证据闭环" in item for item in result.errors), result.errors)

        missing_cue = copy.deepcopy(shot)
        missing_cue["environment_cues"] = []
        result = delivery.ValidationResult()
        delivery.validate_shot_director_integrity(
            data, missing_cue, log, delivery.fact_lookup(data), result, is_first_scene_shot=True
        )
        self.assertTrue(any("证据闭环" in item for item in result.errors), result.errors)

        wrong_basis = copy.deepcopy(shot)
        wrong_basis["environment_cues"][0]["basis"] = "inevitable_action"
        result = delivery.ValidationResult()
        delivery.validate_shot_director_integrity(
            data, wrong_basis, log, delivery.fact_lookup(data), result, is_first_scene_shot=True
        )
        self.assertTrue(any("证据闭环" in item for item in result.errors), result.errors)

        ssxx_progression = copy.deepcopy(shot)
        ssxx_progression["action_progressions"] = [
            {
                "entity": "SS01",
                "evidence_fact_ids": ["B001-F02"],
                "start": "沉默",
        ...
        result = delivery.ValidationResult()
        delivery.validate_shot_director_integrity(
            data, ssxx_progression, log, delivery.fact_lookup(data), result, is_first_scene_shot=True
        )
        self.assertTrue(any("不得使用 SSxx" in item for item in result.errors), result.errors)

        legacy, legacy_log, legacy_shot = sound_case("2.4.8")
        result = delivery.ValidationResult()
        delivery.validate_shot_director_integrity(
            legacy,
            legacy_shot,
            legacy_log,
            delivery.fact_lookup(legacy),
            result,
            is_first_scene_shot=True,
        )
        self.assertTrue(any("必须填写 action_progressions" in item for item in result.errors), result.errors)

    def test_249_same_batch_inheritance_uses_gate_a_derived_slot(self) -> None:
        data = valid_data_249()
        parent = data["continuity_logs"][0]
        parent["sound_sources"] = {
            "SS01": {"name": "门铃", "visibility": "offscreen", "position": "门外", "state": "silent"}
        }
        child = copy.deepcopy(parent)
        ...
        child["sound_sources"]["SS01"]["state"] = "ringing"
        data["continuity_logs"].append(child)

        gate_a_before = delivery.gate_review_hash(data, "GATE_A")
        gate_b_before = delivery.gate_review_hash(data, "GATE_B")
        child["sound_sources"]["SS01"]["state"] = "echoing"
        self.assertEqual(gate_a_before, delivery.gate_review_hash(data, "GATE_A"))
        self.assertNotEqual(gate_b_before, delivery.gate_review_hash(data, "GATE_B"))
        payload = delivery.gate_review_payload(data, "GATE_A")
        derived = payload["continuity_logs"][1]["sound_sources"]["SS01"]["state"]
        self.assertIn(delivery.DERIVED_INHERITANCE_SLOT, derived)
        self.assertEqual("echoing", child["sound_sources"]["SS01"]["state"])
        ...
        cross_before = delivery.gate_review_hash(data, "GATE_A", "BT02")
        child["sound_sources"]["SS01"]["state"] = "ringing"
        self.assertNotEqual(cross_before, delivery.gate_review_hash(data, "GATE_A", "BT02"))

    def test_249_inheritance_dependency_gate_and_shot_order_are_enforced(self) -> None:
        data = valid_data_249()
        parent = data["continuity_logs"][0]
        parent["sound_sources"] = {
            "SS01": {"name": "门铃", "visibility": "offscreen", "position": "门外", "state": "silent"}
        ...
        data["batch_plan"] = {
            "batches": [
                {"batch_id": "BT01", "scene_ids": ["S01"], "expected_shot_count": 2, "depends_on": []},
                {"batch_id": "BT02", "scene_ids": ["S02"], "expected_shot_count": 1, "depends_on": []},
            ]
        }
        result = delivery.ValidationResult()
        delivery.validate_inheritance_dependencies(data, result, target_scenes={"S02"})
        self.assertTrue(any("depends_on 传递闭包" in item for item in result.errors), result.errors)

        data["batch_plan"]["batches"][1]["depends_on"] = ["BT01"]
        result = delivery.ValidationResult()
        delivery.validate_inheritance_dependencies(
            data, result, target_scenes={"S02"}, require_parent_gate_b=True
        )
        self.assertTrue(any("父批 BT01" in item and "GATE_B" in item for item in result.errors), result.errors)
        ...
        gate_a_snapshot = delivery.gate_a_audit_data(data, "BT02")
        gate_a_logs = {
            item["scene_id"]: item for item in gate_a_snapshot["continuity_logs"]
        }
        result = delivery.ValidationResult()
        delivery.validate_inherited_state_values(
            gate_a_snapshot,
            gate_a_logs,
            delivery.final_scene_documents_from_shots(
                gate_a_logs, gate_a_snapshot["shots"]
            ),
            result,
            target_scenes={"S02"},
            cross_batch_only=True,
        )
        self.assertTrue(any("父场景 S01" in item and "终态" in item for item in result.errors), result.errors)
        child["sound_sources"]["SS01"]["state"] = "ringing"
        result = delivery.ValidationResult()
        ...
        target_shot = copy.deepcopy(data["shots"][0])
        target_shot.update({"shot_no": 3, "scene_id": "S02", "scene": "2 室内 日 内"})
        data["shots"].append(target_shot)
        audit = delivery.gate_a_audit_data(data, "BT02")
        self.assertEqual([1, 2], [shot["shot_no"] for shot in audit["shots"]])

        no_batch = copy.deepcopy(data)
        no_batch["batch_plan"] = None
        no_batch["human_reviews"] = []
        no_batch["shots"] = [
            {"shot_no": 1, "scene_id": "S02"},
            {"shot_no": 2, "scene_id": "S01"},
        ]
        logs = {item["scene_id"]: item for item in no_batch["continuity_logs"]}
        final_documents = {scene: copy.deepcopy(log) for scene, log in logs.items()}
        final_documents["S01"]["sound_sources"]["SS01"]["state"] = "ringing"
        result = delivery.ValidationResult()
        delivery.validate_final_inherited_states(
            no_batch,
            logs,
        ...
        self.assertTrue(any("父场景全部镜头必须早于子场景首镜" in item for item in result.errors), result.errors)
        no_batch["shots"].reverse()
        no_batch["shots"][0]["shot_no"] = 1
        no_batch["shots"][1]["shot_no"] = 2
        result = delivery.ValidationResult()
        delivery.validate_final_inherited_states(
            no_batch,
            logs,
        ...
        self.assertFalse(any("父场景全部镜头必须早于子场景首镜" in item for item in result.errors), result.errors)

    def test_action_progression_requires_declared_related_entity(self) -> None:
        data = valid_data_249()
        shot = data["shots"][1]
        shot["action_progressions"][0]["entity"] = "未登记实体"
        result = delivery.ValidationResult()
        delivery.validate_shot_director_integrity(
            data,
            shot,
            data["continuity_logs"][0],
            delivery.fact_lookup(data),
            result,
            is_first_scene_shot=False,
        )
        self.assertTrue(any("必须精确匹配本场" in item for item in result.errors), result.errors)
        self.assertTrue(any("必须在证据事实" in item for item in result.errors), result.errors)


class GateWorkflow249Tests(unittest.TestCase):
    def test_pending_safety_requires_marker_but_not_premature_approval_text(self) -> None:
        data = valid_data_249()
        shot = copy.deepcopy(data["shots"][0])
        shot.update({"shot_type": "safety", "covered_fact_ids": [], "notes": "[安全镜] 剪辑缓冲。"})
        self.assertTrue(
            delivery.pending_safety_exception(
                data, shot, allow_pending_gate_b_exceptions=True
            )
        )
        self.assertFalse(delivery.approved_safety_exception(data, shot))
        shot["notes"] = "剪辑缓冲。"
        self.assertFalse(
            delivery.pending_safety_exception(
                data, shot, allow_pending_gate_b_exceptions=True
    ...
    def test_pending_camera_and_source_exceptions_warn_only_in_gate_b_preflight(self) -> None:
        cases: list[tuple[str, callable]] = []

        def jump(data: dict) -> None:
            data["shots"][1]["camera_geometry"]["angle_delta_from_previous"] = "minor"
            data["shots"][1]["notes"] += " [跳切说明] 有意制造时间跳跃。"

        def axis_cross(data: dict) -> None:
            data["shots"][1]["camera_geometry"]["axis_side"] = "side_b"
            data["shots"][1]["camera_geometry"]["angle_delta_from_previous"] = "axis_cross"
            data["shots"][1]["notes"] += " [越轴说明] 主观失衡。"

        def source_reuse(data: dict) -> None:
            first, second = data["shots"]
            second["source_span"] = copy.deepcopy(first.get("source_span"))
            second["source_spans"] = copy.deepcopy(first.get("source_spans"))
            second["source_reuse"] = {
                "from_shot_no": 1,
                "reason": "source_line_indivisible",
            }
            second["notes"] += " [原文复用] 同句不可拆。"

        cases.extend([("jump-cut", jump), ("axis-cross", axis_cross), ("source-reuse", source_reuse)])
        for label, configure in cases:
            with self.subTest(label=label):
                data = valid_data_249()
                configure(data)
                pending = delivery.ValidationResult()
                delivery.validate_director_adjacent_pair(
                    data,
                    data["shots"][0],
                    data["shots"][1],
                    pending,
                    allow_pending_gate_b_exceptions=True,
                )
                strict = delivery.ValidationResult()
                delivery.validate_director_adjacent_pair(
                    data, data["shots"][0], data["shots"][1], strict
                )
                self.assertTrue(any("待 Gate B 精确批准" in item for item in pending.warnings), pending.warnings)
                self.assertFalse(any("经 Gate B 批准" in item for item in pending.errors), pending.errors)
                self.assertTrue(strict.errors, strict.errors)

    def test_pending_axis_reversal_warns_but_strict_validation_requires_token(self) -> None:
        data = valid_data_249()
        first, second = data["shots"]
        first["camera_main_image"] = first["camera_main_image"].replace("[平视, 中景, 固定镜头]", "[平视, 中景, 推进]")
        second["camera_main_image"] = second["camera_main_image"].replace(
            "[侧面平视, 中景, 横移跟拍]", "[侧面平视, 中景, 拉远]"
        )
        second["notes"] += " [反转动机] 强调人物退缩。"
        pending = delivery.ValidationResult()
        delivery.validate_adjacent_pair(
            data,
            first,
            second,
            {},
            pending,
            guard_reverse_depth_motion=True,
            allow_marked_exceptions=True,
            allow_pending_gate_b_exceptions=True,
        )
        strict = delivery.ValidationResult()
        delivery.validate_adjacent_pair(
        ...
        self.assertTrue(any("axis-reversal" in item for item in pending.warnings), pending.warnings)
        self.assertTrue(any("approved_items" in item for item in strict.errors), strict.errors)

    def test_later_batch_constraint_does_not_change_earlier_batch_hash(self) -> None:
        data = valid_data_249()
        data["batch_plan"] = {
            "batches": [
                {"batch_id": "BT01", "scene_ids": ["S01"], "expected_shot_count": 2, "depends_on": []},
        ...
        before_a = delivery.gate_review_hash(data, "GATE_A", "BT01")
        before_b = delivery.gate_review_hash(data, "GATE_B", "BT01")
        data["directorial_constraints"] = [
            {
                "constraint_id": "DC999",
                "type": "forbid_literal",
        ...
        self.assertEqual(before_a, delivery.gate_review_hash(data, "GATE_A", "BT01"))
        self.assertEqual(before_b, delivery.gate_review_hash(data, "GATE_B", "BT01"))

    def test_dependency_closure_replaces_physical_batch_prefix(self) -> None:
        data = valid_data_249()
        data["batch_plan"] = {
            "batches": [
                {"batch_id": "BT02", "scene_ids": ["S02"], "expected_shot_count": 1, "depends_on": []},
                {"batch_id": "BT01", "scene_ids": ["S01"], "expected_shot_count": 2, "depends_on": []},
                {"batch_id": "BT03", "scene_ids": ["S03"], "expected_shot_count": 1, "depends_on": ["BT01"]},
            ]
        }
        audit = delivery.gate_b_audit_data(data, "BT03")
        included = [item["batch_id"] for item in audit["batch_plan"]["batches"]]
        self.assertEqual(["BT01", "BT03"], included)

    def test_dependency_gate_b_is_required_and_new_round_invalidates_child(self) -> None:
        data = valid_data_249()
        data["batch_plan"] = {
            "batches": [
                {"batch_id": "BT01", "scene_ids": ["S01"], "expected_shot_count": 2, "depends_on": []},
        ...
        append_review(data, "GATE_0")
        append_review(data, "GATE_A", batch_id="BT02")
        self.assertFalse(delivery.effective_gate_approved(data, "GATE_A", batch_id="BT02"))
        data["human_reviews"].pop()
        append_review(data, "GATE_A", batch_id="BT01")
        append_review(data, "GATE_B", batch_id="BT01")
        append_review(data, "GATE_A", batch_id="BT02")
        self.assertTrue(delivery.effective_gate_approved(data, "GATE_A", batch_id="BT02"))
        append_review(data, "GATE_B", batch_id="BT01", round_no=2)
        self.assertFalse(delivery.effective_gate_approved(data, "GATE_A", batch_id="BT02"))

    def test_prompt_overlength_keep_is_gate_c_warn_and_split_invalidates_gate_b(self) -> None:
        data = valid_data_249()
        data["shots"][0]["camera_main_image"] += "\n" + "A反复确认门口、桌边、墙面和手部动作。" * 80
        append_review(data, "GATE_A")
        append_review(data, "GATE_B")
        ...
        pending = delivery.ValidationResult()
        delivery.validate_prompt(data["shots"][0], pending)
        warning = next(item for item in pending.warnings if "超过800字符" in item)
        self.assertIn("Gate C 前", warning)
        self.assertIn("拆镜时回流所属批次 Gate B", warning)
        delivery.validate_warn_resolutions(data, pending, require_all=True)
        self.assertTrue(any("WARN 缺少处置记录" in item for item in pending.errors), pending.errors)

        resolve_warnings(data, pending)
        kept = delivery.ValidationResult(warnings=[warning])
        delivery.validate_warn_resolutions(data, kept, require_all=True)
        self.assertFalse(kept.errors, kept.errors)
        next(
            item
            for item in data["warn_resolutions"]
            if item["warn_id"] == delivery.warning_id(warning)
        )["resolution"] = "revise"
        unresolved = delivery.ValidationResult(warnings=[warning])
        delivery.validate_warn_resolutions(data, unresolved, require_all=True)
        self.assertTrue(any("resolution=revise" in item for item in unresolved.errors), unresolved.errors)

        data["shots"][0]["camera_main_image"] += "\n拆镜后的新画面结构。"
        delivery.derive_prompts(data)
        self.assertFalse(delivery.effective_gate_approved(data, "GATE_B"))


class ContractSlimming249Tests(unittest.TestCase):
    def test_249_rejects_removed_adjacent_marker_and_token_but_248_keeps_legacy_token(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data = valid_data_249()
            approved = root / "approved" / "sample.approved_script.txt"
            ...
            self.assertTrue(any("2.4.9 禁止 [必拆相邻]" in item for item in result.errors), result.errors)

        current = valid_data_249()
        append_review(current, "GATE_A")
        append_review(current, "GATE_B", approved_items=["shot:2:required-adjacent"])
        current_result = delivery.ValidationResult()
        delivery.validate_human_reviews(current, current_result, final_signoff=False)
        self.assertTrue(any("非法逐镜例外 token" in item for item in current_result.errors), current_result.errors)

        historical = valid_data_248()
        append_review(historical, "GATE_A")
        append_review(historical, "GATE_B", approved_items=["shot:2:required-adjacent"])
        historical_result = delivery.ValidationResult()
        delivery.validate_human_reviews(historical, historical_result, final_signoff=False)
        self.assertFalse(
            any("非法逐镜例外 token" in item for item in historical_result.errors),
            historical_result.errors,
        )

    def test_249_rhythm_aggregates_are_warn_only_and_local_structure_still_fails(self) -> None:
        data = valid_data_249()
        for shot in data["shots"]:
            shot["duration_seconds"] = 6
        rhythm = delivery.ValidationResult()
        delivery.validate_overcompression(data, rhythm)
        self.assertFalse(rhythm.errors, rhythm.errors)
        self.assertTrue(any("cut/min" in item for item in rhythm.warnings), rhythm.warnings)
        self.assertTrue(any("短镜比例" in item for item in rhythm.warnings), rhythm.warnings)
        self.assertTrue(any("长镜比例" in item and "高于 55%" in item for item in rhythm.warnings), rhythm.warnings)

        local = valid_data_249()
        local["shots"][0]["beat_ids"] = ["B001", "B002", "B003"]
        local["shots"][0]["covered_fact_ids"] = ["B001-F01", "B002-F01", "B003-F01"]
        local["beats"][0]["facts"][0]["cut_priority"] = "must_isolate"
        local_result = delivery.ValidationResult()
        delivery.validate_overcompression(local, local_result)
        self.assertTrue(any("覆盖 3 个 Beat" in item for item in local_result.errors), local_result.errors)
        self.assertFalse(
            any("cut/min" in item or "短镜比例" in item or "长镜比例" in item for item in local_result.errors),
            local_result.errors,
        )

    def test_249_duration_marker_is_optional_while_duration_breakdown_remains_authoritative(self) -> None:
        shot = copy.deepcopy(valid_data_249()["shots"][0])
        shot["duration_seconds"] = 7
        shot["duration_breakdown"] = {
            "sync_action_seconds": 7,
            "sync_dialogue_seconds": 0,
            "non_sync_action_seconds": 0,
            "emotional_pause_seconds": 0,
        }
        shot["notes"] = "无自然语言估算标签。"
        current = delivery.ValidationResult()
        delivery.validate_duration(
            shot,
            current,
            version_24=True,
            strict_contract=True,
            duration_marker_optional=True,
        )
        self.assertFalse(any("时长估算" in item for item in current.errors), current.errors)

        historical = delivery.ValidationResult()
        delivery.validate_duration(
            shot,
            historical,
            version_24=True,
            strict_contract=True,
            duration_marker_optional=False,
        )
        self.assertTrue(any("时长估算" in item for item in historical.errors), historical.errors)

    def test_249_statuses_exclude_not_run_while_248_remains_readable(self) -> None:
        current = valid_data_249()
        current["validation_report"] = {"status": "NOT_RUN", "source_json_hash": ""}
        current_result = delivery.ValidationResult()
        delivery.validate_metadata(current, current_result, strict_status=True, final_signoff=False)
        self.assertTrue(any("PASS / WARN / FAIL" in item for item in current_result.errors), current_result.errors)

        historical = valid_data_248()
        append_review(historical, "GATE_A")
        append_review(historical, "GATE_B")
        append_review(historical, "GATE_C", notes="accepted_without_validation")
        historical["validation_report"] = {"status": "NOT_RUN", "source_json_hash": ""}
        historical_result = delivery.ValidationResult()
        delivery.validate_metadata(historical, historical_result, strict_status=True, final_signoff=False)
        self.assertFalse(any("status 只允许" in item for item in historical_result.errors), historical_result.errors)

    def test_249_legacy_runtime_receipts_are_ignored_and_excluded_from_hash(self) -> None:
        data = valid_data_249()
        before = delivery.canonical_data_hash(data)
        data["metadata"]["reference_status"] = {"fabricated": "value"}
        data["metadata"]["reference_proof"] = {"fabricated": "value"}
        data["metadata"]["script_status"] = {"fabricated": "value"}
        self.assertEqual(before, delivery.canonical_data_hash(data))
        for key in ("reference_status", "reference_proof", "script_status"):
            data["metadata"].pop(key, None)
        result = delivery.ValidationResult()
        delivery.validate_metadata(data, result, strict_status=False, final_signoff=False)
        self.assertFalse(
            any("reference_status" in item or "reference_proof" in item or "script_status" in item for item in result.errors),
            result.errors,
        )

    def test_249_build_succeeds_without_runtime_receipts(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            data, data_path, md_path, xlsx_path, report_path = prepare_current_fixture(root)
            for key in ("reference_status", "reference_proof", "script_status"):
            ...
            self.assertTrue(all(path.exists() for path in (data_path, md_path, xlsx_path, report_path)))

    def test_249_package_check_requires_five_references_but_not_node_or_project_notes(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            (root / "scripts").mkdir()
            (root / "references").mkdir()
            (root / "VERSION").write_text(delivery.VERSION + "\n", encoding="utf-8")
            (root / "SKILL.md").write_text(
                f"<!-- skill-version: {delivery.VERSION} -->\n{delivery.RULE_REVISION}\n",
                encoding="utf-8",
            )
            (root / "scripts" / "storyboard_delivery.py").write_text(
                f'VERSION = "{delivery.VERSION}"\nRULE_REVISION = "{delivery.RULE_REVISION}"\n',
                encoding="utf-8",
            )
            marker = f"<!-- for skill-version: {delivery.VERSION} / rule-revision: {delivery.RULE_REVISION} -->\n"
            for key in delivery.CURRENT_REQUIRED_REFERENCE_KEYS:
                (root / "references" / f"{key}.md").write_text(marker, encoding="utf-8")
            passed = delivery.validate_current_skill_package(root)
            self.assertFalse(passed.errors, passed.errors)
            missing = root / "references" / "gate-workflow.md"
            missing.unlink()
            failed = delivery.validate_current_skill_package(root)
            self.assertTrue(any("gate-workflow.md" in item for item in failed.errors), failed.errors)

    def test_249_expected_shot_count_is_optional_and_not_in_gate_zero_hash(self) -> None:
        data = valid_data_249()
        data["batch_plan"] = {
            "batches": [{"batch_id": "BT01", "scene_ids": ["S01"], "depends_on": []}]
        }
        result = delivery.ValidationResult()
        delivery.validate_batch_plan(data, result)
        self.assertFalse(any("expected_shot_count" in item for item in result.errors), result.errors)
        before = delivery.gate_review_hash(data, "GATE_0")
        data["batch_plan"]["batches"][0]["expected_shot_count"] = 99
        self.assertEqual(before, delivery.gate_review_hash(data, "GATE_0"))
        data["batch_plan"]["batches"][0]["expected_shot_count"] = 0
        invalid = delivery.ValidationResult()
        delivery.validate_batch_plan(data, invalid)
        self.assertTrue(any("expected_shot_count" in item for item in invalid.errors), invalid.errors)

        historical = valid_data_248()
        historical["batch_plan"] = {
            "batches": [{"batch_id": "BT01", "scene_ids": ["S01"], "depends_on": []}]
        }
        historical_result = delivery.ValidationResult()
        delivery.validate_batch_plan(historical, historical_result)
        self.assertTrue(any("expected_shot_count" in item for item in historical_result.errors), historical_result.errors)

    def test_249_gate_b_review_package_auto_derives_emotion_performance_summary(self) -> None:
        data = valid_data_249()
        fact = data["beats"][0]["facts"][0]
        fact["type"] = "emotion"
        fact["performers"] = ["A"]
        data["shots"][0]["environment_cues"] = [
            {
                "kind": "sound",
                "basis": "fact",
                "description": "门外钟声压低",
        ...
        package = delivery.review_package_text(
            data,
            gate="GATE_B",
            batch_id=None,
            result=delivery.ValidationResult(),
        )
        self.assertIn("自动派生情绪与表演摘要", package)
        self.assertIn("B001-F01:A站在门口。", package)
        self.assertIn("A:手扶门框", package)
        self.assertIn("sound/fact:门外钟声压低", package)
        self.assertIn("门口→桌边", package)

    def test_249_active_docs_do_not_reintroduce_removed_current_semantics(self) -> None:
        skill_root = SCRIPT_DIR.parent
        paths = [
            skill_root / "SKILL.md",
            skill_root / "agents" / "openai.yaml",
        ...
        for token in (
            ...
            "metadata.reference_proof",
            "metadata.script_status",
            "三文件",
        ):
            self.assertNotIn(token, active_text)
        self.assertNotRegex(active_text, r"必须[^\n]{0,30}\[时长估算\]")
