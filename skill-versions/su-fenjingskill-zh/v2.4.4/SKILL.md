...
当前版本：2.4.4。
当前规则修订：`2.4.4-emotion-performance-guard-2026-07-16`。
...
1. 技能根目录下 `VERSION` 为 `2.4.4`。
2. `SKILL.md` 当前规则修订为 `2.4.4-emotion-performance-guard-2026-07-16`。
...
本文件其余部分的所有规则均指 2.4.4 本版规则。`build` 只允许生成 2.4.4 数据；旧版本仅允许 `validate` 只读校验。缺失、未知、未来版本或与版本不匹配的 `rule_revision` 一律 FAIL，不得按最近版本猜测执行。旧数据兼容只按下表处理：
...
| 2.4.4 | 本版全部规则生效；保留 2.4.3 合同，新增有依据的情绪可拍化规则、对白区域豁免与精确执行数值守卫。 |
...
- 2.4.4 的非对白描述不得使用毫米、厘米、米、Hz/K 或精确角度等执行数值；改用一臂距离、隔桌相对、低角度、冷暖倾向和可见结果。镜头秒数、年龄、画幅比例与原样对白中的数值不受此限制。
...
2.4.4 的 `build` 与 `validate` 都必须显式传入 `--workspace-root` 和 `--report`；2.4.3 及更旧版本只能执行只读 `validate`。每次提交 Gate 审核前，先用只读 `review-hash` 生成当前审核范围的 hash；该命令不修改文件，也不代表内容已通过校验：
...
2.4.4 校验器沿用 2.4.3 的真实源文、引用、人工门禁、hash、事实覆盖、时长、WARN 与连续性严格检查，并额外检查非对白区域的文学化禁词与精确执行数值。`source_json_hash` 等于 `canonical_data_hash`：排除 `validation_report`、`human_reviews`、`metadata.revision_log`，包含 `warn_resolutions`；Gate C `reviewed_hash` 必须等于该 hash。strict validate 重算真实结果，不接受陈旧或自称 PASS 的报告。
...
4. 增加情绪可拍化证据边界、1–3 个高信号表现、状态动作链、倾听者与环境反馈约束。
5. 使用相对站位和可见光线结果；2.4.4 非对白区域禁止精确执行数值。
...
6. Gate B 审核包增加情绪表演摘要，但不新增 JSON 字段或 `approved_items` token。
...
5. `画面内容` 保留已锁定的情绪表演、声音质感与合法环境反馈，明确排除六段式总提示词、1900 字上限和缩减台词。
...
| 情绪可拍化 + 对白扫描豁免 + 精确执行数值守卫 | 情绪表演抽象化、对白误伤与伪精确参数 |
