...
本文件是 `su-image9` v2.0.2 的 canon 编译源。`SKILL.md` 正文内联副本是最终仲裁；本文件必须与正文锁块同步。v2.0.2 未改动锁块正文，但同步升级 canon-version 以标记场景/层级感知分页、page-map 派生、标注后处理与门禁行为变化。

## 编译规则

- `final_image_prompts.md` 可使用 `@CANON(NAME)` 标记；validator 必须在 `final_image_prompts.compiled.md` 中展开。
- 最终可生图文本中出现任何 `@CANON(` 字面量，均为硬性失败。
- validator 只能做加法校验：查缺、查漂移、查冲突；不得删冗余、砍长度或剔除重复强化句。
...
`forbidden_prompt_tokens_extra` 只能追加，不能删减 canon 内置硬禁。v2.0.2 的硬禁扫描仅作用于 `CONTINUITY_LAYER` 与 `PANEL_LAYER`，锁块本身不参与扫描。
