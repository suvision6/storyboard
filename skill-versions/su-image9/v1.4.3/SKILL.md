...
1.4.3 新增可互换项目视觉层：题材时代、服装体系、角色外观、道具归属和禁漂移方向必须进入 `PROJECT_VISUAL_PROFILE`，由用户本轮说明、参考图/角色资产、项目资料、连续性台账和 `shot_data.json` 临时派生，不得把某个项目的现代、古代、科幻、民国或奇幻设定写成全局默认。心电图/心跳波形可作为抽象视觉元素出现，但不得生成可读数字、`bpm`、监护仪 UI、网格数据或屏幕文字。
...
- `PROJECT_VISUAL_PROFILE` 是可替换项目视觉层，不是固定项目设定。它只锁定当前任务的题材时代、服装体系、角色外观、道具归属和禁漂移方向；不得把任一项目或任一参考图的具体人物服装写成 `su-image9` 全局默认。
...
## PROJECT_VISUAL_PROFILE｜可互换项目视觉层

`PROJECT_VISUAL_PROFILE` 是 `su-image9` 的项目级内容锁定层。它必须位于 `SYSTEM_STYLE_LAYER` 之后、`SCENE_LAYER` 之前。它不改变黑白石墨分镜媒介，只负责把当前任务的题材时代、服装体系、角色外观、道具归属、道具例外和禁漂移方向转写成可执行约束。

`su-image9` 不内置任何项目的固定题材或固定人物服装。现代、古代、民国、科幻、现实主义、奇幻等都必须来自当前任务输入、参考图、项目资料或连续性资料。若当前任务没有足够资料锁定题材时代，只能写 `PROJECT ERA / GENRE: unresolved from source`，并在语义规划中标注风险；不得自行把角色改成古风、现代或科幻。

### PROFILE_SOURCE_AND_PRECEDENCE

生成 `PROJECT_VISUAL_PROFILE` 前必须先列出来源优先级。优先级固定为：
...
若来源之间冲突，例如用户说现代但参考图是古装、角色资产与连续性台账的道具归属冲突、同名角色有多套不可兼容服装，必须先输出 `任务失败：PROJECT_VISUAL_PROFILE 来源冲突` 并列出冲突点；不得静默混合。

### PROJECT_VISUAL_PROFILE 固定结构

最终提示词必须包含以下短块。完整分析可放在中文分析区，Image2 生图块只保留绘图必要短句：

```text
PROFILE_SOURCE_AND_PRECEDENCE:
Current profile derived from: user instruction / reference assets / project docs / shot_data continuity / inference.
Conflict status: no conflict, or list conflicts and stop.

PROJECT_VISUAL_PROFILE:
PROJECT ERA / GENRE: current project-specific era and genre, derived from sources, not a skill default.
COSTUME SYSTEM: allowed clothing system and forbidden unsourced drift directions.
CHARACTER PROFILE LOCKS: each recurring character's identity, era/genre appearance, hair silhouette, costume silhouette, prop ownership, state changes, screen-position continuity, and forbidden swaps.
PROP PROFILE LOCKS: each recurring prop's owner, shape, allowed state changes, forbidden mis-ownership, and whether it is allowed to influence the surrounding era/genre.
DRIFT GUARD: if the profile is modern, forbid unsourced ancient/fantasy costume drift; if ancient, forbid unsourced modern clothing/devices; if sci-fi/republic-era/realist/fantasy, generate the equivalent project-specific forbidden drift list.
```

角色档案必须按通用模板生成，不得写死某个项目人物：

| 字段 | 必填内容 |
|---|---|
| 身份 | 角色名、年龄/性别/身份关系，只使用来源允许的信息 |
| 时代/题材外观 | 当前 profile 下的外观体系，例如现代、古代、民国、科幻等 |
| 发型轮廓 | 可稳定区分角色的发型、头部轮廓、饰物或禁用饰物 |
| 服装轮廓 | 可在黑白石墨中辨认的服装剪影、层次、材质明暗关系 |
| 道具归属 | 角色可持有或不可持有的关键道具 |
| 状态变化 | 受伤、消散、半透明、倒地、发光等只在来源要求时出现 |
| 屏幕位置 | 本页或本段的 screen left/right、前后层级、可移动范围 |
| 禁止互换 | 不得被画成哪些角色，不得持有哪些错误道具，不得继承哪些错误状态 |

道具档案必须按通用模板生成：

| 字段 | 必填内容 |
|---|---|
| 归属者 | 道具属于谁；若无人持有，写清所在空间或画外状态 |
| 外形 | 黑白石墨可识别的形状、尺寸、材质、关键纹理 |
| 可变化状态 | 裂纹、发光、断裂、脱落、消散等来源允许状态 |
| 禁止误归属 | 不得被哪些角色持有或佩戴 |
| 题材影响权限 | 道具是否允许改变服装/发型/场景时代；默认不得改变 |

每个 Panel 的 `CHARACTER ANCHORS` 只引用本格可见角色的短锚点，例如 `Character A: profile lock A, prop owner, screen left, no prop B/no state C`。完整人物服装和道具档案不得在每格重复，避免提示词超过 12,000 字符并导致噪点、脏线或深色堆叠。

### 抽象心电图/心跳波形规则

心电图线、心跳波形、音频节奏线等可以作为抽象视觉元素出现在画面内。它们不等同于文字或数字，但必须满足：

- 只能是非可读的抽象波形、线条、脉冲或节奏图形。
- 不得出现可读数字、倒计时、`bpm`、`HR`、监护仪 UI、坐标网格、医疗屏幕标签、中文/英文说明、面板数据或设备界面。
- 若画面需要黑场心跳，可写 `abstract ECG-like heartbeat waveform as a non-readable visual rhythm line`；不得写成真实监护仪屏幕。
...
2. `PROJECT_VISUAL_PROFILE`
   - 负责当前任务的题材时代、服装体系、人物外观、道具归属、禁漂移方向和来源优先级。
   - 不重新定义绘画风格，不把某一项目设定写成全局默认。
3. `SCENE_LAYER`
...
- `MUST NOT SHOW` 必须包括：不在本格 `visible_characters` 的角色、不在本格 `visible_props` 的道具、未出现的怪物形态、未来状态、回忆空间、手术室空间、字幕、页眉、镜号、倒计时数字、可读心率数字、监护仪 UI、屏幕文字和标签。抽象 ECG-like 心跳波形可作为非可读视觉元素出现，不得作为文字/数字禁令一刀切排除。
...
- `CHARACTER ANCHORS` 每格只写可见角色的短锚点，例如 `Character A: profile lock A, prop owner, screen right, no prop B/no state C`；完整角色外观与道具锚点表只放在 `PROJECT_VISUAL_PROFILE` 和中文分析区。
...
人物连续页必须输出 `角色外观与道具锚点表`，并在每个相关 Panel 的 `CHARACTER ANCHORS` 中复述本格可见人物短锚点。锚点必须来自 `PROJECT_VISUAL_PROFILE`、`shot_data.json`、连续性台账、参考图或用户当前指令，不得把任一项目的固定人物设定写成全局默认。

角色锚点必须覆盖：

- `identity`：角色名、身份关系、年龄/性别等来源允许信息。
- `era/genre appearance`：当前 `PROJECT_VISUAL_PROFILE` 下的时代/题材外观，不得自行跨题材漂移。
- `hair silhouette`：可区分角色的头部轮廓、发型、饰物或禁用饰物。
- `costume silhouette`：可在黑白石墨中辨认的服装剪影、层次、材质明暗关系。
- `prop ownership`：可持有与不可持有的关键道具。
- `state continuity`：受伤、消散、半透明、倒地、发光等只在来源要求时出现。
- `screen position`：本页或本段 screen left/right、前后层级和允许移动范围。
- `forbidden swaps`：不得被画成哪些角色，不得持有哪些错误道具，不得继承哪些错误状态。

道具锚点必须覆盖：

- `owner / location`：道具属于谁；无人持有时写清空间位置或画外状态。
- `shape lock`：黑白石墨可识别的形状、尺寸、材质和关键纹理。
- `state changes`：裂纹、发光、断裂、脱落、消散等来源允许状态。
- `forbidden ownership`：不得被哪些角色持有、佩戴、误触或误归属。
- `era influence permission`：道具是否允许影响服装/发型/场景时代；默认不得影响。古旧、科幻或仪式性道具只能改变自身外形，不能无来源地改变人物服装或项目题材。

题材漂移防线必须从当前 `PROJECT_VISUAL_PROFILE` 自动生成：

- 当前 profile 是现代题材时，禁止无来源古装、仙侠、武侠、发簪、发髻、飘带长袖、古风铠甲等漂移。
- 当前 profile 是古代题材时，禁止无来源现代西装、现代皮衣、智能设备、汽车、枪械、城市办公空间等漂移。
- 当前 profile 是民国、科幻、现实主义、奇幻或其他题材时，必须根据 profile 生成等价禁漂移项。
- 若某个道具本身来自另一时代或风格，必须在 `PROP PROFILE LOCKS` 中写清它只是道具例外，不得把人物服装、发型或场景整体带向该道具的时代/风格。

每个 Panel 的 `CHARACTER ANCHORS` 只写本格可见人物短锚点；完整 profile 不得逐格重复。当某格不应出现某角色或道具时，必须在 `MUST NOT SHOW` 中显式写入。若同一页中无法用 profile 和短锚点防止人物互换，应拆页或请求补充参考，不得硬生成。
...
参考图流程必须先生成 `PROFILE_SOURCE_AND_PRECEDENCE` 和 `PROJECT_VISUAL_PROFILE`。角色图和道具图优先用于锁定身份、服装轮廓、发型轮廓、道具形状和道具归属；不得把参考图的照片质感、颜色、棚拍光、AI 精修或渲染风格带入 `SYSTEM_STYLE_LAYER`。
...
- 角色/道具参考图与用户本轮明确题材、项目文档或连续性台账冲突，且无法按优先级解决。
...
- 最终提示词应写 `Use references only for identity, costume silhouette, prop shape, and spatial facts; inherit PROJECT_VISUAL_PROFILE and SYSTEM_STYLE_LAYER.` 或等价短句。

### 纯文字输入

当输入只有剧本文字、纯剧情段落、文字版分镜、Markdown/Excel 表格内容、表格截图转写内容、局部镜号或镜号范围时，按纯文字流程执行。

纯文字流程也必须生成 `PROJECT_VISUAL_PROFILE`。若剧本或分镜文字明确时代、服装、职业、空间、道具体系，则按来源锁定；若没有明确资料，只能把 profile 标为未解析或弱推断，不得自行补成现代、古代、科幻或奇幻。纯文字流程不得要求用户补参考图；不得为了画面丰富新增角色、新道具、新建筑、新动作结果或新对白。如果输入中出现参考图或资产线索，切换到参考图流程。
...
- 必须先生成 `PROFILE_SOURCE_AND_PRECEDENCE` 和 `PROJECT_VISUAL_PROFILE`。缺少 profile、profile 来源冲突未处理、或把某个项目设定写成全局默认时，必须停止。
- 当前页人物、服装、发型、道具归属和禁漂移方向必须来自 `PROJECT_VISUAL_PROFILE`；不得让道具或参考图风格把整个项目题材无来源地改成现代、古代、科幻、民国、奇幻或其他风格。
...
- 人物连续页缺少 `角色外观与道具锚点表`，或当前 profile 中的连续人物没有在 Panel 内写入 `CHARACTER ANCHORS` 时，必须停止。
- 任一连续人物的身份、轮廓、服装、道具归属、状态或屏幕位置与来源相互错置，例如角色持有他人专属道具、继承他人专属身体标记、被画成错误角色，必须停止。
- 任一连续人物的身份、时代/题材外观、服装轮廓、道具归属、状态或屏幕位置与 `PROJECT_VISUAL_PROFILE` 相互错置，必须停止；错误示例包括现代 profile 中无来源古装漂移、古代 profile 中无来源智能设备漂移、角色持有他人专属道具、道具把全页服装拉向自身时代。
...
- 必须存在完整 `PROJECT_VISUAL_PROFILE`，且必须位于 `SYSTEM_STYLE_LAYER` 之后、`PANEL_LAYER P01-P09` 之前；缺失、后置或被 Panel 层替代都视为语义规划失败。
...
- 参考图内容与 `PROJECT_VISUAL_PROFILE` 冲突时，必须按 `PROFILE_SOURCE_AND_PRECEDENCE` 解决；无法解决时输出 `任务失败：PROJECT_VISUAL_PROFILE 来源冲突`。
...
- 人物连续页是否输出 `角色外观与道具锚点表`，并在 Panel 内锁定当前 `PROJECT_VISUAL_PROFILE` 中连续角色的轮廓、服装、道具归属、状态和屏幕位置。
...
3. `PROFILE_SOURCE_AND_PRECEDENCE`
4. `PROJECT_VISUAL_PROFILE`
5. `SCENE_LAYER`
6. `CAMERA_RULE_LAYER`
7. `CONTINUITY_LAYER`
8. `REFERENCE_USAGE` 或 `TEXT_DERIVED_LAYOUT`
9. `PANEL_1_MASTER_SPATIAL_LAYOUT_ANCHOR`
10. `DOOR_WINDOW_FURNITURE_GEOMETRY_LOCK`
11. `VEHICLE_AND_AXIS_LOCKS`
12. `OBJECT_VISIBILITY_AND_BOUNDARIES`
13. `PANEL_LAYER P01-P09`
14. `NEGATIVE_CONSTRAINTS`

结构规则：

- `SYSTEM_STYLE_LAYER` 必须出现在 `PANEL_LAYER P01-P09` 之前。
- `PROFILE_SOURCE_AND_PRECEDENCE` 与 `PROJECT_VISUAL_PROFILE` 必须出现在 `SYSTEM_STYLE_LAYER` 之后、`SCENE_LAYER` 之前。
- `PROJECT_VISUAL_PROFILE` 只写当前任务可替换题材/角色/道具锁，不得写成 `su-image9` 固定默认。
...
若某页没有车辆、对视反打或参考图，对应中文表格可以写“本页不适用”，但不得删除 `SYSTEM_STYLE_LAYER`、`PROFILE_SOURCE_AND_PRECEDENCE`、`PROJECT_VISUAL_PROFILE`、`风格继承检查表`、`PANEL_LAYER P01-P09` 或 `NEGATIVE_CONSTRAINTS`。

## 生图与验收

只有用户明确要求生图或 ZIP 时才生图。每段提示词生成一张独立 3x3 九宫格 PNG，PNG 放入 `pages/`，与 `prompts.md` 打包 ZIP。

原始生图目检顺序固定为：整体横版 16:9、不得是 3:2/4:3/横版 A4、3x3 九格、9 格同宽同高、每格 16:9、无方格/竖窄格/混合尺寸宫格、无画内文字、允许非可读抽象心跳波形但不得有可读数字/UI、整体为 monochrome graphite storyboard、像 hand-drawn pencil / graphite sketch、无数字厚涂、无 CG、无电影光、无漫画页、无日漫渲染、9 格同一 stroke weight、9 格同一 shading density、9 格同一 mid-gray tonal range、存在 `PROJECT_VISUAL_PROFILE`、人物服装/发型/道具归属匹配当前 profile、无来源题材漂移、Panel 1 主平面锚定、每格人物身份正确、`VISIBLE ONLY` 无多画角色、`MUST NOT SHOW` 无误入画面、镜头三要素匹配 `MUST MATCH SHOT_DATA CAMERA TAG`、固定物几何继承、车辆局部坐标、座位-车窗邻接、车内摄影机占位、车内外同侧窗口、反打轴线、屏幕左右不越轴、对象可见性。任一失败，收紧提示词并重生一次；最多连续重生两次。
...
多页九宫格、批量生成、按镜号范围生成多个 Markdown prompt 或 ZIP 时，所有页必须使用完全相同的 `SYSTEM_STYLE_LAYER`。同一项目/同一角色资产批次还必须继承同一个 `PROJECT_VISUAL_PROFILE`，除非来源明确转场到另一时代、回忆层、异空间或换装段落；场景切换只能改变 `SCENE_LAYER`，不得改变媒介、线条、灰度、颗粒、渲染密度或分镜师手感。现实/幻境差异只允许通过构图、对象透明度、空间关系表达，不得改成彩色、CG、电影光、厚涂、漫画或高反差风格。批量验收必须检查多页是否像同一位分镜师同一生产场次画出，人物和道具是否继承同一 profile。

生图前还必须完成语义验收和风格验收：重复格、Panel 1 非锚定、跨空间混页、泛化轴线、车辆坐标缺失、缺少 `SYSTEM_STYLE_LAYER`、缺少 `PROJECT_VISUAL_PROFILE`、profile 来源冲突未解决、人物/道具归属与 profile 冲突、Panel 层覆盖风格、参考风格冲突、batch 风格未锁定、渲染禁令不足任一存在时，不得生图。
