---
name: su-image9
description: Image2/gpt-image-2 9 宫格 3x3 黑白石墨铅笔分镜提示词独立技能。v2.0.0 融合版采用"生成层合同不可侵犯 + 治理层可降级"双层架构：SKILL 正文内联携带 SYSTEM_STYLE_LAYER、几何蓝图、负面约束、硬词清单四个锁块全文作为权威兜底副本；canon-locks.md 与 validator 存在时走编译注入与机器门禁（G0/G1），缺失时显式降级为内联全文 + 落盘自查表模式，任何情况下最终生图文本不得出现未展开的 @CANON 标记。Panel 采用双轨制：panel_plan.json 短字段供机器比对，生图文本 PANEL_LAYER 必须为自然语言视觉描述。字符预算只设下限不设硬性上限，冗余强化视为设计特性。人工确认点条件触发（HC-1 仅降级/多页时、HC-3 与模糊回复不推进无条件保留），单页无降级任务走快速通道。保留 1.7.x 的 PAGE/PANEL 命名、重试收敛、page-map、缺页规则、manifest、batch_id、R-BIND 矩阵与失败分类。回归标准以"同题出图不劣于 1.4.0 基线"为最高验收。不得修改 su-fenjingskill-zh 主表、Prompt 列、Storyboard 列、Excel 或校验脚本。
---
...
**2.0.0 融合原则**：1.4.0 的生成层内容（风格锁全文、几何蓝图、负面约束、硬词清单、领域空间规则、自然语言 Panel 描述、冗余强化）全部回迁 SKILL 正文并定义为不可侵犯合同；1.7.3 的治理机制（canon 编译、机器门禁、人工确认、重试收敛、manifest、batch）全部降级为"存在即用、缺失即有定义好的退路"的可选增强层。**治理层的任何规则不得从生成层减掉一个 token。**
...
- 后处理前必须逐格核对生成图实际构图与 JSON 三要素、原分镜剧情；不符时**不得用假标签迁就错误画面**，该格标记 `构图不符，需重生/人工确认`。
...
- 仅含正集 → 可直用；含负集 → 必须按 DOM-1 改写；**两者皆含或皆不含 → warn，列入降级声明交人工裁定**，禁止静默二选一。
...
- G1 事实门禁（full 模式）：逐格比对 shot_data ↔ panel_plan ↔ compiled prompt：来源镜号、camera tag、drawn camera tag、可见性、轴线完整性、道具归属、锚定触发合规、拆格差异、continuity 已读取。
...
1. **出图对照基线**：固定 3 组代表性剧本（含车辆页、对视反打页、纯文字 sparse 页），以 1.4.0 提示词产出的图为基线；v2.0.0 产出必须在 IMG-01 至 IMG-12 逐项**不劣于**基线，方可视为合入成功。
