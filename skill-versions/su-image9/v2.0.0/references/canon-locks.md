...
本文件是 `su-image9` v2.0.0 的 canon 编译源。`SKILL.md` 正文内联副本是最终仲裁；本文件必须与正文锁块同步。
...
- 修改任一 canon 块必须同步更新 `SKILL.md` 内联副本并升 `canon-version`。
...
`forbidden_prompt_tokens_extra` 只能追加，不能删减 canon 内置硬禁。v2.0.0 的硬禁扫描仅作用于 `CONTINUITY_LAYER` 与 `PANEL_LAYER`，锁块本身不参与扫描。
