...
HYBRID_VERSIONS = {"2.3.4", "2.3.5", "2.3.6"}
ADJACENT_MOTION_GUARD_VERSIONS = {"2.3.5", "2.3.6"}
STRUCTURED_AUDIT_VERSIONS = {"2.3.6"}
...
def fact_structural_cut_key(fact: dict[str, Any]) -> tuple[str, str, str]:
    return (
        fact_beat_id(fact),
...
def structural_cut_groups(facts: list[dict[str, Any]], *, must_only: bool) -> dict[tuple[str, str, str], list[str]]:
    groups: dict[tuple[str, str, str], list[str]] = {}
    for fact in facts:
        if must_only and clean_text(fact.get("cut_priority")) != "must_isolate":
            continue
        key = fact_structural_cut_key(fact)
        if not all(key):
            continue
...
def structured_key_label(key: tuple[str, str, str]) -> str:
    return "/".join(key)
...
def validate_metadata(data: dict[str, Any], result: ValidationResult, *, strict_status: bool) -> None:
    ...
    if strict_status and requires_structured_audit(data) and report.get("source_json_hash"):
...
def validate_overcompression(data: dict[str, Any], result: ValidationResult) -> None:
    ...
    for shot in shots:
        ...
        structural_groups = structural_cut_groups(must_facts, must_only=False)
        cut_points = find_required_cut_point_categories(
            clean_text(shot.get("source_paragraph")) + "\n" + clean_text(shot.get("camera_main_image"))
        )
