...
def read_version(skill_dir: Path, audit: Audit) -> str:
    ...
    if version != SKILL_VERSION:
        audit.fail(
            "F-VERSION",
            "VERSION",
            "validator only implements the 2.1.0 contract",
...
def source_gate(data: dict[str, Any], source_path: Path, plan: dict[str, Any] | None, audit: Audit) -> None:
    ...
    if metadata.get("version") != UPSTREAM_VERSION:
        audit.fail("F-SOURCE-VERSION", "shot_data.metadata.version", "only the exact 2.4.2 upstream contract is supported", expected=UPSTREAM_VERSION, actual=metadata.get("version"))
    if metadata.get("rule_revision") != UPSTREAM_RULE_REVISION:
        audit.fail("F-SOURCE", "shot_data.metadata.rule_revision", "upstream rule revision does not match 2.4.2", expected=UPSTREAM_RULE_REVISION, actual=metadata.get("rule_revision"))
    ...
    if not isinstance(lock, dict):
        ...
        if isinstance(locked_text, str) and isinstance(locked_hash, str):
            fingerprint = re.sub(r"\s+", "", locked_text.strip())
            expected_lock_hash = sha256_bytes(fingerprint.encode("utf-8"))
            if locked_hash != expected_lock_hash:
                audit.fail("F-SOURCE-LOCK", "shot_data.script_lock.locked_text_hash", "locked script hash mismatch", expected=expected_lock_hash, actual=locked_hash)
    ...
    if not isinstance(source_json_hash, str) or not source_json_hash:
        audit.fail("F-SOURCE-HASH", "shot_data.validation_report.source_json_hash", "source_json_hash is required")
    elif source_json_hash != expected_content_hash:
        audit.fail("F-SOURCE-HASH", "shot_data.validation_report.source_json_hash", "source content hash mismatch", expected=expected_content_hash, actual=source_json_hash)
