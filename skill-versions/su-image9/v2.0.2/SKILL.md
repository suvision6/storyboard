---
name: su-image9
description: Image2/gpt-image-2 9 宫格 3x3 黑白石墨铅笔分镜提示词独立技能。v2.0.2 融合版采用"生成层合同不可侵犯 + 治理层可降级"双层架构：SKILL 正文内联携带 SYSTEM_STYLE_LAYER、几何蓝图、负面约束、硬词清单四个锁块全文作为权威兜底副本；canon-locks.md 与 validator 存在时走编译注入与机器门禁（G0/G1），缺失时显式降级为内联全文 + 落盘自查表模式，任何情况下最终生图文本不得出现未展开的 @CANON 标记。Panel 采用双轨制：panel_plan.json 短字段供机器比对，生图文本 PANEL_LAYER 必须为自然语言视觉描述。v2.0.2 强化正式派生脚本、Panel 1 源顺序锁、相邻位移距离锁、场景/层级感知分页和标注版 PDF 后处理，不再固定每 9 镜头硬切，禁止回忆/手术/黑屏等不可锚定层级镜头静默开页。字符预算只设下限不设硬性上限，冗余强化视为设计特性。人工确认点条件触发（HC-1 仅降级/多页时、HC-3 与模糊回复不推进无条件保留），单页无降级任务走快速通道。保留 1.7.x 的 PAGE/PANEL 命名、重试收敛、page-map、缺页规则、manifest、batch_id、R-BIND 矩阵与失败分类。回归标准以"同题出图不劣于 1.4.0 基线"为最高验收。不得修改 su-fenjingskill-zh 主表、Prompt 列、Storyboard 列、Excel 或校验脚本。
---
...
**2.0.2 融合原则**：1.4.0 的生成层内容（风格锁全文、几何蓝图、负面约束、硬词清单、领域空间规则、自然语言 Panel 描述、冗余强化）全部回迁 SKILL 正文并定义为不可侵犯合同；1.7.3 的治理机制（canon 编译、机器门禁、人工确认、重试收敛、manifest、batch）全部降级为"存在即用、缺失即有定义好的退路"的可选增强层。**治理层的任何规则不得从生成层减掉一个 token。** v2.0.2 同步升级 skill-version 与 canon-version，并把正式派生、源顺序锁、距离阶段锁、场景/层级感知分页、真实 C 号标注与标注边框检测纳入机器门禁。
...
- 标注脚本必须优先检测实际 3x3 宫格边框位置；检测失败时可回退 canonical boxes，但必须在 manifest 写 warning，不得静默裁切或错位。
...
- 正式派生脚本不得固定每 9 镜头硬切；必须执行 `scene_layer_aware_strict_source_order` 分页。回忆层、手术现实层、黑屏等不适合锚定的镜头不得作为页首，除非人工确认并另行记录；终局慢动作/倒计时若是该段主空间建立镜头，可作为该页首格，但必须声明层级 bridge。
- 同一页跨 `scene_id` 或跨叙事层级时，`panel_plan.json` 必须显式写入 `page_split_policy`：跨同空间当前层场景写 `intentional_cross_scene_bridge`，跨回忆/当前等层级写 `intentional_layer_bridge`；未声明视为门禁失败。
...
该脚本必须产出 `分析与锁定.md`、`panel_plan.json`、`page-map.json`、`final_image_prompts.md`、`final_image_prompts.compiled.md`、`validation_report.json`，并严格保持来源镜头顺序。临时派生脚本不得绕过 v2.0.2 的顺序锁、距离阶段锁与场景/层级感知分页。
...
- G1 事实门禁（full 模式）：逐格比对 shot_data ↔ panel_plan ↔ compiled prompt：来源镜号、camera tag、drawn camera tag、可见性、轴线完整性、道具归属、锚定触发合规、拆格差异、continuity 已读取、`G1-12` 来源镜号顺序锁、`G1-13` 相邻位移距离阶段锁、`G1-14` 场景/层级分页策略锁。
...
- 后处理调用 `annotate_storyboard_pages.py`；脚本报错 = F-ASSET；脚本缺失时标注版降级为"人工按 GEN-8 规则排版"并在交付说明中声明。标注 manifest 必须保留 `panel_labels`、`box_detection` 与 warnings。
...
1. **出图对照基线**：固定 3 组代表性剧本（含车辆页、对视反打页、纯文字 sparse 页），以 1.4.0 提示词产出的图为基线；v2.0.2 产出必须在 IMG-01 至 IMG-12 逐项**不劣于**基线，方可视为合入成功。
