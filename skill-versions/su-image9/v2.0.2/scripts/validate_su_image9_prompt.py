#!/usr/bin/env python3
"""Validator/compiler for su-image9 v2.0.2 final prompts."""
...
def require_text(page: Page, text: str, source: str, check: str = "G0-04") -> None:
    ...
    if normalize_for_compare(text) not in normalize_for_compare(page_text):
        page.findings.append(issue(check, "fail", f"{page.name}/{source}", "missing", "required v2.0.2 generation-layer text missing"))
...
def shot_layer_key(shot: dict[str, Any]) -> str:
    text = " ".join(
        [
            str(shot.get("scene", "")),
...
def validate_page_split_policy(page: Page, pplan: dict[str, Any], source_numbers: list[int], shots: dict[int, dict[str, Any]]) -> None:
    ...
    if not policy:
        page.findings.append(issue("G1-14", "fail", f"{page.name}/page_split_policy", "missing", "v2.0.2 requires scene/layer-aware page split policy"))
        return
    if "scene_layer_aware_strict_source_order" not in policy:
        page.findings.append(issue("G1-14", "fail", f"{page.name}/page_split_policy", policy, "page split policy must be scene/layer aware and preserve source order"))
    ...
    if not source_shots:
        return
    first_layer = shot_layer_key(source_shots[0])
    if first_layer in BAD_PAGE_START_LAYERS:
        page.findings.append(
            issue(
                "G1-14",
                "fail",
                f"{page.name}/PANEL-1/source_layer",
                first_layer,
                "memory, surgery, or black transition layers cannot open a page; split earlier or use manual confirmation outside validator",
            )
        )
    scene_ids = unique_in_order([str(shot.get("scene_id", "")) for shot in source_shots])
    layers = unique_in_order([shot_layer_key(shot) for shot in source_shots])
    if len(scene_ids) > 1 and "intentional_cross_scene_bridge" not in policy:
        page.findings.append(
            issue(
                "G1-14",
                "fail",
                f"{page.name}/page_split_policy",
                policy,
                f"page crosses scene_id values {','.join(scene_ids)} without intentional_cross_scene_bridge",
            )
        )
    if len(layers) > 1 and "intentional_layer_bridge" not in policy:
        page.findings.append(
            issue(
                "G1-14",
                "fail",
                f"{page.name}/page_split_policy",
                policy,
                f"page crosses narrative layers {','.join(layers)} without intentional_layer_bridge",
            )
        )
...
def validate_g0(pages: list[Page], compiled_text: str, panel_plan: dict[str, Any]) -> None:
    ...
    for page in pages:
        ...
        if re.search(r"(?m)^(STYLE_LOCK|CANVAS_LOCK|REFERENCE_LOCK|PANEL_TASKS|NEGATIVE_LOCK):", page.text):
            page.findings.append(issue("G0-10", "fail", page.name, "v1.7.3 structure", "old PANEL_TASKS/canon-lock structure is not allowed for v2.0.2 final prompts"))
        ...
        for index in range(1, 10):
            ...
            for field_name in MACHINE_PANEL_FIELDS:
                if pp.get(field_name) in {None, ""}:
                    page.findings.append(issue("G1-02", "fail", f"{page.name}/PANEL-{index}/{field_name}", "missing", "required v2.0.2 machine-track field missing"))
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compile and validate su-image9 v2.0.2 prompts.")
