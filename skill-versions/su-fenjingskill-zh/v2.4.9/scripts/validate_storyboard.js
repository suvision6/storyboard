...
const VERSION = "2.4.9";
const RULE_REVISION = "2.4.9-gate-state-contract-2026-07-21";
...
function usage() {
  ...
  console.error(
    `Legacy compatibility wrapper bundled with ${VERSION} (${RULE_REVISION}). ` +
      "The 2.4.9 formal entry is storyboard_delivery.py; this file remains for read-only historical workflows."
  );
}
...
const python = args.python || process.env.CODEX_PYTHON;
if (!python) {
  usage();
  console.error("Missing --python (or controlled CODEX_PYTHON)");
  process.exit(2);
}
