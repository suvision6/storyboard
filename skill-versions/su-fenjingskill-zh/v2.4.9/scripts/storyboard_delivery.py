...
VERSION_PROFILES: dict[str, Any] = {
    ...
    "2.4.6": {
        "rule_revision": "2.4.6-dialogue-partition-guard-2026-07-20",
        "contract_integrity": True,
        "emotion_performance_guard": True,
        "director_integrity_guard": True,
        "dialogue_partition_guard": True,
    },
    "2.4.7": {
        "rule_revision": "2.4.7-validation-integrity-guard-2026-07-20",
        "contract_integrity": True,
        "emotion_performance_guard": True,
        "director_integrity_guard": True,
        "dialogue_partition_guard": True,
        "validation_integrity_guard": True,
    },
    "2.4.8": {
        "rule_revision": "2.4.8-stage-integrity-guard-2026-07-21",
        "contract_integrity": True,
        "emotion_performance_guard": True,
        "director_integrity_guard": True,
        "dialogue_partition_guard": True,
        "validation_integrity_guard": True,
        "stage_integrity_guard": True,
    },
    "2.4.9": {
...
for _version, _profile in list(VERSION_PROFILES.items()):
    ...
    _profile.setdefault("dialogue_partition_guard", False)
    _profile.setdefault("validation_integrity_guard", False)
    _profile.setdefault("stage_integrity_guard", False)
    _profile.setdefault("gate_workflow_guard", False)
...
CURRENT_VALID_STATUSES = {"PASS", "WARN", "FAIL"}
LEGACY_VALID_STATUSES = {"PASS", "WARN", "FAIL", "NOT_RUN"}
...
GATE_WORKFLOW_REFERENCE_KEY = "gate-workflow"
CURRENT_REQUIRED_REFERENCE_KEYS = REQUIRED_REFERENCE_KEYS + (GATE_WORKFLOW_REFERENCE_KEY,)
...
CURRENT_NOTE_MARKERS = [marker for marker in NOTE_MARKERS if marker not in {"[必拆相邻]", "[reference missing]"}]
DEPRECATED_CURRENT_METADATA_KEYS = ("reference_status", "reference_proof", "script_status")
...
DERIVED_INHERITANCE_SLOT = "$derived_inheritance"
...
def validate_current_skill_package(skill_root: Path | None = None) -> ValidationResult:
    """Fail closed when the active 2.4.9 rule package is incomplete or mixed."""

    result = ValidationResult()
    root = (skill_root or Path(__file__).resolve().parent.parent).resolve(strict=False)
    version_path = root / "VERSION"
    script_path = root / "scripts" / "storyboard_delivery.py"
    skill_path = root / "SKILL.md"
    try:
        installed_version = version_path.read_text(encoding="utf-8-sig").strip()
    except (OSError, UnicodeError) as exc:
        result.error(f"当前 Skill 包 VERSION 无法读取：{exc}")
    else:
        if installed_version != VERSION:
            result.error(f"当前 Skill 包 VERSION 必须为 {VERSION}，实际为 {installed_version or '<empty>'}。")
    try:
        script_text = script_path.read_text(encoding="utf-8-sig")
    except (OSError, UnicodeError) as exc:
        result.error(f"当前 Skill 包缺少正式 Python 入口 scripts/storyboard_delivery.py：{exc}")
    else:
        if f'VERSION = "{VERSION}"' not in script_text or f'RULE_REVISION = "{RULE_REVISION}"' not in script_text:
            result.error("正式 Python 入口的 VERSION / RULE_REVISION 与活动规则包不一致。")
        if skill_root is None and script_path.resolve(strict=False) != Path(__file__).resolve(strict=False):
            result.error("当前执行的 storyboard_delivery.py 不属于所解析的 Skill 根目录。")
    try:
        skill_text = skill_path.read_text(encoding="utf-8-sig")
    except (OSError, UnicodeError) as exc:
        result.error(f"当前 Skill 包 SKILL.md 无法读取：{exc}")
    else:
        if f"skill-version: {VERSION}" not in skill_text or RULE_REVISION not in skill_text:
            result.error("SKILL.md 的版本或规则修订与正式 Python 入口不一致。")
    expected_reference_marker = f"for skill-version: {VERSION} / rule-revision: {RULE_REVISION}"
    for key in CURRENT_REQUIRED_REFERENCE_KEYS:
        reference_path = root / "references" / f"{key}.md"
        try:
            reference_text = reference_path.read_text(encoding="utf-8-sig")
        except (OSError, UnicodeError) as exc:
            result.error(f"当前 Skill 包缺少必需 reference：references/{key}.md（{exc}）")
            continue
        if expected_reference_marker not in reference_text:
            result.error(f"references/{key}.md 的版本或规则修订与活动规则包不一致。")
    return result


def extend_result(target: ValidationResult, source: ValidationResult) -> None:
    target.errors.extend(message for message in source.errors if message not in target.errors)
...
def data_without_validation_report(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if is_current_contract(data):
        ...
        if metadata:
            ...
            if uses_gate_workflow_guard(data):
                for deprecated_key in DEPRECATED_CURRENT_METADATA_KEYS:
...
def batch_dependencies(data: dict[str, Any]) -> dict[str, set[str]]:
    """Return the declared batch DAG without assigning meaning to list order."""

    return {
        clean_text(batch.get("batch_id")): set(normalized_list(batch.get("depends_on")))
        for batch in as_list(as_dict(data.get("batch_plan")).get("batches"))
        if isinstance(batch, dict) and clean_text(batch.get("batch_id"))
    }


def transitive_batch_dependencies(data: dict[str, Any], batch_id: str | None) -> list[str]:
    """Return a deterministic dependency closure; malformed cycles stay finite."""

    target = clean_text(batch_id)
    if not target:
        return []
    graph = batch_dependencies(data)
    seen: set[str] = set()
    pending = list(graph.get(target, set()))
    while pending:
        current = pending.pop()
...
def constraint_scene_ids(data: dict[str, Any], constraint: dict[str, Any]) -> set[str]:
    """Resolve the scenes whose reviewed content is constrained by one item."""

    constraint_type = clean_text(constraint.get("type"))
    if constraint_type == "direct_handoff":
        target = clean_text(constraint.get("to_scene_id"))
        return {target} if target else set()
    scenes = set(normalized_list(constraint.get("scene_ids")))
    beat_scenes = {
        clean_text(fact.get("fact_id")): clean_text(beat.get("scene_id"))
    ...
    for fact_id in normalized_list(constraint.get("fact_ids")):
        scene_id = beat_scenes.get(fact_id)
        if scene_id:
            scenes.add(scene_id)
    shot_scenes = {
        shot.get("shot_no"): clean_text(shot.get("scene_id"))
        for shot in as_list(data.get("shots"))
        if isinstance(shot, dict)
    }
    for shot_no in as_list(constraint.get("shot_nos")):
        scene_id = shot_scenes.get(shot_no)
        if scene_id:
            scenes.add(scene_id)
    # Keep malformed references visible to schema validation, but never let an
    # unresolved scope contaminate every batch hash.
    return {scene for scene in scenes if scene}


def constraint_in_scene_scope(
    data: dict[str, Any], constraint: dict[str, Any], scene_scope: set[str] | None
) -> bool:
    if scene_scope is None:
        return True
    scope = constraint_scene_ids(data, constraint)
    return bool(scope and scope & scene_scope)
...
def inheritance_uses_gate_b_slot(
    data: dict[str, Any], child_scene: Any, parent_scene: Any
) -> bool:
    """Return whether inherited values are unknown until the child's Gate B."""

    if not uses_gate_workflow_guard(data):
        return False
    child_batch = batch_id_for_scene(data, child_scene)
    parent_batch = batch_id_for_scene(data, parent_scene)
    return child_batch is None or parent_batch is None or child_batch == parent_batch


def replace_inherited_values_with_derived_slots(
    data: dict[str, Any], continuity_logs: Any
) -> list[dict[str, Any]]:
    """Replace same-batch inherited values in a hash copy, never in delivery JSON."""

    values = [copy.deepcopy(item) for item in as_list(continuity_logs) if isinstance(item, dict)]
    for child in values:
        child_scene = clean_text(child.get("scene_id"))
        parent_scene = clean_text(child.get("inherits_from"))
        if not parent_scene or not inheritance_uses_gate_b_slot(data, child_scene, parent_scene):
            continue
        for raw_pointer in as_list(child.get("inherited_states")):
            try:
                ...
                # Structural validation reports the malformed path.  The hash
                # helper stays total so review-hash can fail with diagnostics.
                continue
    return values


def gate_review_payload(
    data: dict[str, Any],
    gate: str,
    batch_id: str | None = None,
    *,
    _hash_stack: frozenset[str] = frozenset(),
) -> dict[str, Any]:
    ...
    if gate == "GATE_0":
        batch_plan = copy.deepcopy(data.get("batch_plan"))
        if uses_gate_workflow_guard(data) and isinstance(batch_plan, dict):
            for batch in as_list(batch_plan.get("batches")):
                if isinstance(batch, dict):
                    batch.pop("expected_shot_count", None)
        base["batch_plan"] = batch_plan
        return base
    if gate in {"GATE_A", "GATE_B"}:
        base["batch_id"] = clean_text(batch_id)
        scoped_logs = _items_for_scenes(data.get("continuity_logs"), scenes)
        base["continuity_logs"] = (
            replace_inherited_values_with_derived_slots(data, scoped_logs)
            if gate == "GATE_A" and uses_gate_workflow_guard(data)
            else scoped_logs
        )
        ...
        if uses_director_integrity_guard(data):
            ...
            constraints = [
                ...
                and (
                    not uses_gate_workflow_guard(data)
                    or constraint_in_scene_scope(data, item, scenes)
                )
            ]
            base["directorial_constraints"] = constraints
        if uses_gate_workflow_guard(data) and batch_id:
            base["dependency_gate_b_hashes"] = [
                {
                    "batch_id": dependency,
                    "reviewed_content_hash": _gate_review_hash(
                        data,
                        "GATE_B",
                        dependency,
                        _hash_stack | frozenset({clean_text(batch_id)}),
                    ),
                }
                for dependency in transitive_batch_dependencies(data, batch_id)
            ]
    if gate == "GATE_B":
...
def _gate_review_hash(
    data: dict[str, Any], gate: str, batch_id: str | None, hash_stack: frozenset[str]
) -> str:
    normalized_batch = clean_text(batch_id) or None
    if normalized_batch and normalized_batch in hash_stack:
        return hashlib.sha256(b"cyclic-batch-review-payload").hexdigest()
    try:
        if clean_text(gate) == "GATE_C":
            return final_content_hash(data)
        payload = json.dumps(
            gate_review_payload(data, gate, normalized_batch, _hash_stack=hash_stack),
            ensure_ascii=False,
            sort_keys=True,
            separators=(",", ":"),
            allow_nan=False,
...
def gate_review_hash(data: dict[str, Any], gate: str, batch_id: str | None = None) -> str:
    return _gate_review_hash(data, gate, batch_id, frozenset())
...
def report_summary_items(data: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    if not uses_gate_workflow_guard(data):
        items.append(
            (
                "reference_status",
                json.dumps(as_dict(metadata.get("reference_status")), ensure_ascii=False, sort_keys=True),
            )
        )
...
def uses_dialogue_partition_guard(data: dict[str, Any]) -> bool:
    profile = version_profile(data)
    return bool(profile and profile.get("dialogue_partition_guard"))


def uses_validation_integrity_guard(data: dict[str, Any]) -> bool:
    profile = version_profile(data)
    return bool(profile and profile.get("validation_integrity_guard"))


def uses_stage_integrity_guard(data: dict[str, Any]) -> bool:
    profile = version_profile(data)
    return bool(profile and profile.get("stage_integrity_guard"))


def uses_gate_workflow_guard(data: dict[str, Any]) -> bool:
    profile = version_profile(data)
    return bool(profile and profile.get("gate_workflow_guard"))
...
def latest_review_with_index(
    data: dict[str, Any], gate: str, batch_id: str | None = None
) -> tuple[int, dict[str, Any]] | None:
    candidates = [
        (index, review)
        for index, review in enumerate(as_list(data.get("human_reviews")))
    ...
    if not candidates:
        return None
    return max(candidates, key=lambda item: int(item[1]["round"]))


def latest_review(data: dict[str, Any], gate: str, batch_id: str | None = None) -> dict[str, Any] | None:
    item = latest_review_with_index(data, gate, batch_id)
    return item[1] if item is not None else None


def _latest_review_matches_current_content(
    data: dict[str, Any], gate: str, batch_id: str | None
) -> bool:
    item = latest_review_with_index(data, gate, batch_id)
    if item is None:
        return False
    review = item[1]
    return (
        clean_text(review.get("status")) == "approved"
        and clean_text(review.get("reviewed_hash")) == gate_review_hash(data, gate, batch_id)
    )


def effective_gate_approved(
    data: dict[str, Any],
    gate: str,
    *,
    batch_id: str | None = None,
    _seen: frozenset[tuple[str, str | None]] = frozenset(),
) -> bool:
    """Return the single authoritative current-state Gate decision."""

    normalized_batch = clean_text(batch_id) or None
    key = (clean_text(gate), normalized_batch)
    if key in _seen or not _latest_review_matches_current_content(data, key[0], normalized_batch):
        return False
    if not uses_gate_workflow_guard(data):
        return True
    current_record = latest_review_with_index(data, key[0], normalized_batch)
    if current_record is None:
        return False
    current_index = current_record[0]
    batches = sorted(batch_dependencies(data))
    prerequisites: list[tuple[str, str | None]] = []
    if key[0] == "GATE_A":
        if batches:
            prerequisites.append(("GATE_0", None))
            prerequisites.extend(
                ("GATE_B", dependency)
                for dependency in sorted(batch_dependencies(data).get(normalized_batch or "", set()))
            )
    elif key[0] == "GATE_B":
        prerequisites.append(("GATE_A", normalized_batch))
        prerequisites.extend(
            ("GATE_B", dependency)
            for dependency in sorted(batch_dependencies(data).get(normalized_batch or "", set()))
        )
    elif key[0] == "GATE_C":
        prerequisites.extend(("GATE_B", item) for item in batches)
        if not batches:
            prerequisites.append(("GATE_B", None))
    for prerequisite in prerequisites:
        record = latest_review_with_index(data, prerequisite[0], prerequisite[1])
        if (
            record is None
            or record[0] >= current_index
            or not effective_gate_approved(
                data,
                prerequisite[0],
                batch_id=prerequisite[1],
                _seen=_seen | frozenset({key}),
            )
        ):
            return False
    return True
...
def approved_review(
    ...
    *,
    ...
    if is_current_contract(data):
        ...
        if review is None or not effective_gate_approved(data, gate, batch_id=batch_id):
            return False
...
def approved_safety_exception(data: dict[str, Any], shot: dict[str, Any]) -> bool:
    ...
    if is_current_contract(data):
        ...
        return (
            ...
            and "[安全镜]" in notes
            and (not uses_gate_workflow_guard(data) or not normalized_list(shot.get("covered_fact_ids")))
            and (uses_gate_workflow_guard(data) or "[人工批准]" in notes)
            and approved_review(
                data,
                "GATE_B",
                batch_id=batch_id,
...
def pending_safety_exception(
    data: dict[str, Any], shot: dict[str, Any], *, allow_pending_gate_b_exceptions: bool
) -> bool:
    return bool(
        uses_gate_workflow_guard(data)
        and allow_pending_gate_b_exceptions
        and clean_text(shot.get("shot_type")) == "safety"
        and not normalized_list(shot.get("covered_fact_ids"))
        and "[安全镜]" in clean_text(shot.get("notes"))
    )
...
def update_validation_report(
    ...
    if not uses_gate_workflow_guard(data):
        data["validation_report"]["reference_status"] = as_dict(metadata.get("reference_status"))
    if uses_director_integrity_guard(data):
    ...
    *,
    ...
    if not uses_gate_workflow_guard(data):
        ...
        if requires_human_gate(data):
            ...
            for key in ("storyboard_delivery.py", "validate_storyboard.js"):
    ...
    if requires_human_gate(data):
        revision_log = metadata.get("revision_log")
        if not isinstance(revision_log, list):
            result.error("metadata.revision_log 必须是数组。")
        elif uses_validation_integrity_guard(data):
            allowed_keys = {"round", "trigger", "summary"}
            for index, entry in enumerate(revision_log, start=1):
                label = f"metadata.revision_log[{index - 1}]"
                if not isinstance(entry, dict):
                    result.error(f"{label} 必须是对象。")
                    continue
                unknown = sorted(set(entry) - allowed_keys)
                if unknown:
                    result.error(f"{label} 含未知字段：{', '.join(unknown)}。")
                if set(entry) != allowed_keys:
                    missing = sorted(allowed_keys - set(entry))
                    if missing:
                        result.error(f"{label} 缺少字段：{', '.join(missing)}。")
                round_value = entry.get("round")
                if not is_json_integer(round_value, minimum=1) or int(round_value) != index:
                    result.error(f"{label}.round 必须从 1 开始连续递增；期望 {index}。")
                for field_name in ("trigger", "summary"):
                    raw_value = entry.get(field_name)
                    value = clean_text(raw_value)
                    if not isinstance(raw_value, str) or not value or raw_value != value:
    ...
    valid_statuses = CURRENT_VALID_STATUSES if uses_gate_workflow_guard(data) else LEGACY_VALID_STATUSES
    if strict_status and status is not None and status not in valid_statuses:
        allowed = "PASS / WARN / FAIL" if uses_gate_workflow_guard(data) else "PASS / WARN / FAIL / NOT_RUN"
        result.error(f"validation_report.status 只允许 {allowed}。")
    if not uses_gate_workflow_guard(data) and requires_human_gate(data) and clean_text(status) == "NOT_RUN":
...
def validate_batch_plan(
    ...
    result: ValidationResult,
    *,
    required_collections: tuple[str, ...] = ("continuity_logs", "beats", "shots"),
) -> dict[str, set[str]]:
    ...
    for index, batch in enumerate(raw_batches, start=1):
        ...
        if expected is None and not uses_gate_workflow_guard(data):
            result.error(f"{batch_id} expected_shot_count 必须是大于0的整数。")
        elif expected is not None and not is_json_integer(expected, minimum=1):
            result.error(f"{batch_id} expected_shot_count 必须是大于0的整数。")
    ...
    if uses_gate_workflow_guard(data):
        ...
        for constraint in as_list(data.get("directorial_constraints")):
            if not isinstance(constraint, dict) or clean_text(constraint.get("type")) != "direct_handoff":
                continue
            source_batch = batch_id_for_scene(
                data, fact_scenes.get(clean_text(constraint.get("from_fact_id")))
            )
            target_batch = batch_id_for_scene(data, constraint.get("to_scene_id"))
            if (
                source_batch
                and target_batch
                and source_batch != target_batch
                and source_batch not in set(transitive_batch_dependencies(data, target_batch))
            ):
                result.error(
                    f"{constraint.get('constraint_id')} direct_handoff 跨批来源 {source_batch} "
                    f"必须位于目标批次 {target_batch} 的 depends_on 传递闭包。"
                )
        validate_inheritance_dependencies(data, result)
    registered_scenes = {
    ...
    present_by_collection = {
        "continuity_logs": log_scenes,
        "beats": beat_scenes,
        "shots": shot_scenes,
    }
    for label in required_collections:
        present = present_by_collection[label]
...
def validate_human_reviews(data: dict[str, Any], result: ValidationResult, *, final_signoff: bool) -> None:
    ...
    for review_index, review in enumerate(reviews):
        ...
        elif uses_gate_workflow_guard(data) and "accepted_without_validation" in clean_text(review.get("notes")):
            result.error("2.4.9 不支持 accepted_without_validation；未完成最终校验时保持未签发。")
        ...
        if not isinstance(approved_items, list) or any(
            ...
            not re.fullmatch(
                (
                    r"shot:[1-9][0-9]*:(?:safety|axis-reversal|jump-cut|axis-cross|performance-chain-break|source-reuse)"
                    if uses_gate_workflow_guard(data)
                    else r"shot:[1-9][0-9]*:(?:safety|axis-reversal|required-adjacent|jump-cut|axis-cross|performance-chain-break|source-reuse)"
                    if uses_director_integrity_guard(data)
        ...
        elif uses_gate_workflow_guard(data) and gate == "GATE_B" and status == "rejected" and approved_items:
            result.error("GATE_B rejected 记录的 approved_items 必须为空数组。")
    ...
    if batch_ids:
        for batch_id in sorted(batch_ids):
            ...
            if uses_gate_workflow_guard(data):
                for dependency in sorted(batch_dependencies(data).get(batch_id, set())):
                    require_latest_after(("GATE_B", dependency), ("GATE_A", batch_id))
...
def validate_report_truth(
    ...
    *,
    ...
    if status == "NOT_RUN" and not uses_gate_workflow_guard(data):
...
def set_json_pointer_value(document: Any, pointer: Any, value: Any) -> None:
    """Set an existing JSON Pointer target without creating delivery fields."""
    ...
    if isinstance(current, dict) and last in current:
        ...
        return
    if isinstance(current, list) and re.fullmatch(r"0|[1-9][0-9]*", last):
        index = int(last)
        if index < len(current):
...
def final_scene_documents_from_shots(
    continuity_logs: dict[str, dict[str, Any]], shots: Any
) -> dict[str, dict[str, Any]]:
    """Derive each scene's terminal ledger from its ordered shot updates."""

    documents = {scene: copy.deepcopy(log) for scene, log in continuity_logs.items()}
    ordered_shots = sorted(
        (shot for shot in as_list(shots) if isinstance(shot, dict)),
        key=lambda shot: shot.get("shot_no") if is_json_integer(shot.get("shot_no"), minimum=1) else math.inf,
    )
    for shot in ordered_shots:
        document = documents.get(clean_text(shot.get("scene_id")))
        if document is None:
            continue
        for update in as_list(shot.get("continuity_updates")):
            apply_update_to_scene_document(document, update)
    return documents


def validate_inheritance_dependencies(
    data: dict[str, Any],
    result: ValidationResult,
    *,
    target_scenes: set[str] | None = None,
    require_parent_gate_b: bool = False,
) -> None:
    """Validate cross-batch inheritance against the declared execution DAG."""

    if not uses_gate_workflow_guard(data):
        return
    for child in as_list(data.get("continuity_logs")):
        if not isinstance(child, dict):
            continue
        child_scene = clean_text(child.get("scene_id"))
        if target_scenes is not None and child_scene not in target_scenes:
            continue
        parent_scene = clean_text(child.get("inherits_from"))
        if not parent_scene:
            continue
        child_batch = batch_id_for_scene(data, child_scene)
        parent_batch = batch_id_for_scene(data, parent_scene)
        if not child_batch or not parent_batch or child_batch == parent_batch:
            continue
        closure = set(transitive_batch_dependencies(data, child_batch))
        if parent_batch not in closure:
            result.error(
                f"{child_scene} 跨批继承父场景 {parent_scene} 所属 {parent_batch}，"
                f"该批次必须位于 {child_batch} 的 depends_on 传递闭包。"
            )
            continue
        if require_parent_gate_b and not effective_gate_approved(
            data, "GATE_B", batch_id=parent_batch
        ):
            result.error(
                f"{child_scene} 跨批继承父场景 {parent_scene} 前，父批 {parent_batch} "
                "必须有当前有效的 GATE_B approved。"
            )


def validate_inherited_state_values(
    data: dict[str, Any],
    ...
    *,
    target_scenes: set[str] | None = None,
    cross_batch_only: bool = False,
) -> None:
    for scene, child in continuity_logs.items():
        if target_scenes is not None and scene not in target_scenes:
            continue
        parent_scene = clean_text(child.get("inherits_from"))
        if not parent_scene or parent_scene not in continuity_logs:
            continue
        if cross_batch_only and inheritance_uses_gate_b_slot(data, scene, parent_scene):
            continue
...
def validate_final_inherited_states(
    ...
    if not is_current_contract(data):
        return
    validate_inherited_state_values(data, continuity_logs, final_scene_documents, result)
    if not uses_gate_workflow_guard(data):
        return
    validate_inheritance_dependencies(data, result, require_parent_gate_b=True)
    shot_numbers: dict[str, list[int]] = {}
    for shot in as_list(data.get("shots")):
        if not isinstance(shot, dict) or not is_json_integer(shot.get("shot_no"), minimum=1):
            continue
        shot_numbers.setdefault(clean_text(shot.get("scene_id")), []).append(int(shot["shot_no"]))
    for scene, child in continuity_logs.items():
        parent_scene = clean_text(child.get("inherits_from"))
        if not parent_scene or parent_scene not in continuity_logs:
            continue
        parent_numbers = shot_numbers.get(parent_scene, [])
        child_numbers = shot_numbers.get(scene, [])
        if parent_numbers and child_numbers and max(parent_numbers) >= min(child_numbers):
            result.error(
                f"{scene} 继承父场景 {parent_scene} 时，父场景全部镜头必须早于子场景首镜。"
            )
...
def validate_adjacent_shot_design(
    ...
    *,
    ...
    allow_pending_gate_b_exceptions: bool = False,
) -> None:
    ...
    for item in shots:
        ...
        if previous is not None and item_scene == previous_scene:
            validate_adjacent_pair(
                ...
                allow_pending_gate_b_exceptions=allow_pending_gate_b_exceptions,
            )
        previous = item
...
def validate_adjacent_pair(
    ...
    *,
    ...
    validate_director_adjacent_pair(
        data,
        prev,
        curr,
        result,
        ...
        if {prev_depth_direction, curr_depth_direction} == {"push_in", "pull_out"}:
            if allow_marked_exceptions and "[反转动机]" in clean_text(curr.get("notes")):
                if is_current_contract(data) and not approved_review(
                    ...
                    if uses_gate_workflow_guard(data) and allow_pending_gate_b_exceptions:
                        result.warn(
                            f"镜号{curr_no} [反转动机] 已形成 axis-reversal 例外申请，待 Gate B 精确批准。"
                        )
                    else:
...
def validate_shot_director_integrity(
    ...
    *,
    ...
    obligated_visible_performers = {
        performer
        for fact_id in covered
        if isinstance(facts_by_id.get(fact_id), dict)
        and clean_text(facts_by_id[fact_id].get("type")) in {"action", "emotion"}
        for performer in normalized_list(facts_by_id[fact_id].get("performers"))
        if performer in visible
    }
    # 2.4.8's read-only profile already expanded group coverage.  2.4.9 keeps
    # that historical behavior and names the one canonical capacity formula.
    cue_upper_bound = max(3, len(obligated_visible_performers)) if uses_stage_integrity_guard(data) else 3
    if visible and not 1 <= len(cues) <= cue_upper_bound:
        if uses_gate_workflow_guard(data):
            result.error(
                f"镜号{shot_no} 有可见人物时 performance_cues 必须为 1-{cue_upper_bound} 项；"
                "上限唯一公式为 max(3, obligated_visible_performers 数量)。"
            )
        else:
            result.error(
                f"镜号{shot_no} 有可见人物时 performance_cues 必须为 1-{cue_upper_bound} 项；"
                "通常上限为3，群体表演时按有证据义务的可见表演者人数扩展。"
            )
    ...
    continuity_updates = [
        update for update in as_list(shot.get("continuity_updates")) if isinstance(update, dict)
    ]
    progression_updates = (
        [
            update
            for update in continuity_updates
            if clean_text(update.get("entity_type")) != "sound_source"
        ]
        if uses_gate_workflow_guard(data)
        else continuity_updates
    )
    progression_required = bool(progression_updates) or bool(
    ...
    declared_progression_entities = (
        declared
        | declared_entity_names(scene_log, "props")
        | declared_entity_names(scene_log, "fixed_objects")
    )
    reality_layer = clean_text(scene_log.get("reality_layer"))
    if reality_layer:
        declared_progression_entities.add(reality_layer)
    for index, progression in enumerate(progressions, start=1):
        label = f"镜号{shot_no} action_progressions[{index}]"
        if not isinstance(progression, dict):
            result.error(f"{label} 必须是对象。")
            continue
        entity = clean_text(progression.get("entity"))
        if not entity:
            result.error(f"{label}.entity 不能为空。")
        elif uses_gate_workflow_guard(data) and re.fullmatch(SOUND_SOURCE_ID_PATTERN, entity):
            result.error(
                f"{label}.entity 不得使用 SSxx 声源；声源迁移由 sound fact、"
                "sound environment_cue 与 continuity_update 闭环取证。"
            )
        elif uses_stage_integrity_guard(data) and entity not in declared_progression_entities:
            result.error(f"{label}.entity 必须精确匹配本场角色、道具、固定物或现实层台账。")
        evidence = normalized_list(progression.get("evidence_fact_ids"))
        if not evidence or not set(evidence).issubset(covered):
            result.error(f"{label}.evidence_fact_ids 必须是本镜已覆盖事实的非空子集。")
        if uses_stage_integrity_guard(data):
            for fact_id in evidence:
                fact = facts_by_id.get(fact_id)
                if not isinstance(fact, dict):
                    continue
                fact_type = clean_text(fact.get("type"))
                if fact_type in {"dialogue", "space", "character"}:
                    result.error(
                        f"{label}.evidence_fact_ids 不得用 {fact_type} fact 充当状态推进证据：{fact_id}"
                    )
                if entity in declared:
                    performer_match = entity in normalized_list(fact.get("performers"))
                    text_match = entity in clean_text(fact.get("text"))
                    if not performer_match and not text_match:
                        result.error(f"{label}.entity 与证据事实 {fact_id} 没有明确人物关联。")
                elif entity and entity not in clean_text(fact.get("text")):
                    result.error(f"{label}.entity 必须在证据事实 {fact_id} 的 fact.text 中明确出现。")
    ...
    if uses_gate_workflow_guard(data):
        ...
        sound_fact_cue_evidence = [
            set(normalized_list(cue.get("evidence_fact_ids"))) & covered_sound_facts
            for cue in environment
            if isinstance(cue, dict)
            and clean_text(cue.get("kind")) == "sound"
            and clean_text(cue.get("basis")) == "fact"
        ]
        for update in continuity_updates:
            if clean_text(update.get("entity_type")) != "sound_source":
                continue
            update_sound_evidence = (
                set(normalized_list(update.get("evidence_fact_ids"))) & covered_sound_facts
            )
            if not update_sound_evidence or not any(
                update_sound_evidence & cue_evidence
                for cue_evidence in sound_fact_cue_evidence
            ):
                result.error(
                    f"镜号{shot_no} sound_source continuity_update 必须与同一 sound fact "
                    "取证的 kind=sound、basis=fact environment_cue 形成证据闭环。"
                )
...
def validate_director_adjacent_pair(
    ...
    *,
    ...
    if delta in {"same", "minor"}:
        ...
        has_marker = "[跳切说明]" in clean_text(curr.get("notes"))
        if has_marker and approved_review(data, "GATE_B", batch_id=batch_id, approved_item=token):
            result.warn(f"镜号{prev_no}-{curr_no} 有意保留同轴小于等价30度变化的跳切，已登记 [跳切说明]。")
        elif has_marker and uses_gate_workflow_guard(data) and allow_pending_gate_b_exceptions:
            result.warn(
                f"镜号{prev_no}-{curr_no} [跳切说明] 已形成 jump-cut 例外申请，待 Gate B 精确批准。"
            )
        else:
            result.error(
    ...
    if is_axis_cross:
        ...
        has_marker = "[越轴说明]" in clean_text(curr.get("notes"))
        if has_marker and approved_review(data, "GATE_B", batch_id=batch_id, approved_item=token):
            result.warn(f"镜号{prev_no}-{curr_no} 有意越轴，已登记 [越轴说明]。")
        elif has_marker and uses_gate_workflow_guard(data) and allow_pending_gate_b_exceptions:
            result.warn(
                f"镜号{prev_no}-{curr_no} [越轴说明] 已形成 axis-cross 例外申请，待 Gate B 精确批准。"
            )
        else:
    ...
    if prev_signature and prev_signature == curr_signature:
        ...
        if structurally_valid_reuse and approved_review(
            data, "GATE_B", batch_id=batch_id, approved_item=token
        ):
            result.warn(f"镜号{prev_no}-{curr_no} 复用同一原文跨度，已按 [原文复用] 登记并批准。")
        elif structurally_valid_reuse and uses_gate_workflow_guard(data) and allow_pending_gate_b_exceptions:
            result.warn(
                f"镜号{prev_no}-{curr_no} [原文复用] 已形成 source-reuse 例外申请，待 Gate B 精确批准。"
            )
        else:
            result.error(
...
def validate_fact_text_preservation(data: dict[str, Any], result: ValidationResult) -> None:
    ...
    for beat in as_list(data.get("beats")):
        ...
        if not source:
            continue
        facts = as_list(beat.get("facts"))
        strict_dialogue_fact_ids: set[str] = set()
        if uses_validation_integrity_guard(data):
            dialogue_facts = [
                fact
                for fact in facts
                if isinstance(fact, dict) and clean_text(fact.get("type")) == "dialogue"
            ]
            expected_ids = {
            ...
            speakers = known_dialogue_speakers(data, beat)
            source_records = strict_dialogue_records(source, speakers)
            strict_dialogue_fact_ids = dialogue_partition_fact_ids(
                source,
                facts,
                known_speakers=speakers,
            )
            if source_records or dialogue_facts:
                if strict_dialogue_fact_ids != expected_ids:
                    result.error(
                        f"{beat.get('beat_id')} dialogue facts 必须连续、逐字、全量消费所属 Beat 的全部原文对白；"
                        "不得遗漏前后对白、夹入非 dialogue fact、跨对白拼接或改变 speaker/delivery。"
                    )
        elif uses_dialogue_partition_guard(data):
            strict_dialogue_fact_ids = dialogue_partition_fact_ids_246(source, facts)
        for fact in facts:
            if not isinstance(fact, dict):
                continue
            ...
            if is_current_contract(data) and clean_text(fact.get("type")) == "dialogue":
                if uses_validation_integrity_guard(data):
                    continue
                allowed = dialogue_candidates_for_data(data, source)
                fact_text = normalize_script_text(fact.get("text")).strip()
                candidates = dialogue_candidates_for_data(data, fact_text)
                ...
                if (not candidates or not candidates.issubset(allowed)) and fact_id not in strict_dialogue_fact_ids:
...
def validate_overcompression(data: dict[str, Any], result: ValidationResult) -> None:
    ...
    if cut_per_minute and cut_per_minute < 10.5:
        ...
        if has_multi_beat_critical and not uses_gate_workflow_guard(data):
    ...
    if uses_director_integrity_guard(data):
        ...
        if uses_gate_workflow_guard(data) and shots and long_ratio > 0.55:
            result.warn(f"节奏预警：长镜比例 {long_ratio:.2%} 高于 55%，镜头节奏可能过度绵长，必须由人工审核处置。")


def validate_performance_chains_and_constraints(
    data: dict[str, Any],
    result: ValidationResult,
    *,
    allow_pending_gate_b_exceptions: bool = False,
    require_shot_resolution: bool = True,
) -> None:
    if not uses_director_integrity_guard(data):
        return
    ...
    for index, chain in enumerate(as_list(raw_chains), start=1):
        ...
        for step_index, step in enumerate(steps, start=1):
            ...
            for fact_id in evidence:
                ...
                fact = facts[fact_id]
                fact_type = clean_text(fact.get("type"))
                role = clean_text(step.get("role"))
                if uses_stage_integrity_guard(data):
                    if role == "dialogue" and fact_type != "dialogue":
                        result.error(f"{step_label} role=dialogue 只能引用 dialogue fact：{fact_id}")
                    if role in {"action", "reaction"} and fact_type not in {"action", "emotion"}:
                        result.error(
                            f"{step_label} role={role} 只能引用 action / emotion fact：{fact_id}"
                        )
        ...
        if uses_stage_integrity_guard(data):
            ordered_chain_facts = [
                ...
                if fact_id in fact_order
            ]
            positions = [fact_order[fact_id] for fact_id in ordered_chain_facts]
            if any(current <= previous for previous, current in zip(positions, positions[1:])):
                result.error(f"{label}.steps 必须按 Beat/fact 的叙事顺序严格递增。")
        if chain_id:
        ...
        if common_shots:
            continue
        if not require_shot_resolution:
            continue
        approved_break_shots: dict[int, int] = {}
        pending_break_boundaries: set[int] = set()
        for shot in shots:
            shot_no = shot.get("shot_no")
            for item in as_list(shot.get("chain_breaks")):
                ...
                break_is_approved = structurally_valid_break and approved_review(
                        data,
                        "GATE_B",
                        batch_id=batch_id_for_scene(data, shot.get("scene_id")),
                        approved_item=token,
                    )
                if break_is_approved or (
                    structurally_valid_break and allow_pending_gate_b_exceptions
                ):
                    approved_break_shots[int(boundary)] = int(shot_no)
                    if not break_is_approved:
                        pending_break_boundaries.add(int(boundary))
        ...
        if valid_partition:
            for boundary in boundaries:
                if boundary in pending_break_boundaries:
                    result.warn(
                        f"{chain_id} 在步骤 {boundary}/{boundary + 1} 之间拆镜，"
                        "已登记 [动作链拆分]，待 Gate B 精确批准。"
                    )
                else:
                    result.warn(
    ...
    for beat in as_list(data.get("beats")):
        ...
        for fact in as_list(beat.get("facts")):
            ...
            if role is None:
                if uses_stage_integrity_guard(data) or clean_text(fact.get("type")) == "reality":
                    performance_sequence.append(None)
                continue
            if role == "dialogue":
    ...
    for index, constraint in enumerate(as_list(raw_constraints), start=1):
        ...
        if require_shot_resolution and constraint_type == "same_shot" and fact_ids:
        ...
        if constraint_type == "forbid_literal":
            ...
            for shot in shots if require_shot_resolution else []:
...
def validate_rows(
    ...
    *,
    ...
    for shot in shots:
        ...
        pending_safety = pending_safety_exception(
            data,
            shot,
            allow_pending_gate_b_exceptions=allow_pending_gate_b_exceptions,
        )
        if not fact_ids and safety_exception:
            # A valid approval resolves the exception.  Do not invent a new
            # WARN that historical or current contracts would then have to
            # resolve a second time.
            pass
        elif not fact_ids and pending_safety:
            result.warn(
                f"镜号{shot_no} [安全镜] 已形成 safety 例外申请，待 Gate B 精确批准。"
            )
        elif not fact_ids:
        ...
        if is_current_contract(data):
            ...
            if uses_gate_workflow_guard(data) and "[必拆相邻]" in clean_text(shot.get("notes")):
                result.error(
                    f"镜号{shot_no} 2.4.9 禁止 [必拆相邻]；请改为 substantial 机位变化、合并镜头，"
                    "或提交结构合法的 [跳切说明] pending jump-cut 例外。"
                )
            allowed_markers = set(
                CURRENT_NOTE_MARKERS
                if uses_gate_workflow_guard(data)
                else NOTE_MARKERS
                if uses_director_integrity_guard(data)
                else LEGACY_NOTE_MARKERS
            )
            for marker in re.findall(r"\[[^\]\n]+\]", clean_text(shot.get("notes"))):
                if uses_gate_workflow_guard(data) and marker == "[必拆相邻]":
                    continue
        ...
        validate_duration(
            ...
            strict_contract=is_current_contract(data),
            duration_marker_optional=uses_gate_workflow_guard(data),
        )
...
def validate_duration(
    ...
    *,
    ...
    strict_contract: bool = False,
    duration_marker_optional: bool = False,
) -> None:
    shot_no = shot.get("shot_no")
    ...
    notes = clean_text(shot.get("notes"))
    if duration > 6 and "[时长估算]" not in notes and not duration_marker_optional:
...
def validate_prompt(shot: dict[str, Any], result: ValidationResult) -> None:
    ...
    if len(prompt) > 800:
        result.warn(
            f"镜号{shot_no} Prompt 总长超过800字符，须在 Gate C 前由人工处置；"
            "保留时登记 WARN resolution，拆镜时回流所属批次 Gate B。"
        )
...
_LEGACY_DIALOGUE_CANDIDATE_LABEL_RE = re.compile(
    r"[\u4e00-\u9fa5A-Za-z0-9·]{1,8}(?:（VO）|\(VO\)|VO|画外声)?[：:]"
)
_LEGACY_DIALOGUE_LABEL_RE = re.compile(
    r"(?P<speaker>[\u4e00-\u9fa5A-Za-z0-9·]{0,8}?)(?P<vo>（VO）|\(VO\)|\bVO\b|画外声)?[：:]",
    re.IGNORECASE,
)
_LEGACY_VO_MARKER_RE = re.compile(r"(?:（VO）|\(VO\)|\bVO\b|画外声)", re.IGNORECASE)

_DIALOGUE_VO_PATTERN_246 = r"(?:（VO）|\(VO\)|(?<![A-Za-z0-9])VO\b|画外声)"
_DIALOGUE_DIRECTION_PATTERN_246 = r"(?:（[^（）\r\n：:]{1,40}）|\([^()\r\n：:]{1,40}\))"
_DIALOGUE_CANDIDATE_LABEL_RE_246 = re.compile(
    rf"[\u4e00-\u9fa5A-Za-z0-9·]{{1,8}}?(?:{_DIALOGUE_VO_PATTERN_246})?"
    rf"(?:{_DIALOGUE_DIRECTION_PATTERN_246})?[：:]",
    re.IGNORECASE,
)
_DIALOGUE_LABEL_RE_246 = re.compile(
    rf"(?P<speaker>[\u4e00-\u9fa5A-Za-z0-9·]{{0,8}}?)(?P<vo>{_DIALOGUE_VO_PATTERN_246})?"
    rf"(?P<direction>{_DIALOGUE_DIRECTION_PATTERN_246})?[：:]",
    re.IGNORECASE,
)
_VO_MARKER_RE_246 = re.compile(_DIALOGUE_VO_PATTERN_246, re.IGNORECASE)


def legacy_dialogue_candidates(value: Any) -> set[str]:
    text = normalize_script_text(value)
    candidates = {segment for segment in quoted_segments(text) if segment}
    labels = list(_LEGACY_DIALOGUE_CANDIDATE_LABEL_RE.finditer(text))
...
def dialogue_candidates_246(value: Any) -> set[str]:
    text = normalize_script_text(value)
    candidates = {segment for segment in quoted_segments(text) if segment}
    labels = list(_DIALOGUE_CANDIDATE_LABEL_RE_246.finditer(text))
    for index, match in enumerate(labels):
        next_label = labels[index + 1].start() if index + 1 < len(labels) else len(text)
        next_line = text.find("\n", match.end())
        end = min(next_label, next_line if next_line >= 0 else len(text))
...
def declared_dialogue_texts(value: Any) -> list[str]:
    declared = normalize_script_text(value).strip()
    if not declared:
        return []
    if (declared.startswith("“") and declared.endswith("”")) or (
        declared.startswith('"') and declared.endswith('"')
    ):
        return [segment for segment in quoted_segments(declared) if segment]
    return [declared]


def legacy_dialogue_records(value: Any) -> list[tuple[str, str, bool]]:
    text = normalize_script_text(value)
    ...
    for raw_line in text.split("\n"):
        ...
        if not line:
            continue
        labels = list(_LEGACY_DIALOGUE_LABEL_RE.finditer(line))
        if labels:
            for index, match in enumerate(labels):
...
def dialogue_records_246(value: Any) -> list[tuple[str, str, bool]]:
    text = normalize_script_text(value)
    records: list[tuple[str, str, bool]] = []
    pending: tuple[str, bool] | None = None
    for raw_line in text.split("\n"):
        ...
        if not line:
            continue
        labels = list(_DIALOGUE_LABEL_RE_246.finditer(line))
        if labels:
            for index, match in enumerate(labels):
                end = labels[index + 1].start() if index + 1 < len(labels) else len(line)
                speaker = normalize_dialogue_speaker(match.group("speaker"))
                is_vo = bool(match.group("vo"))
                declared = line[match.end():end].strip()
                texts = declared_dialogue_texts(declared)
        ...
        if pending is not None:
            speaker, is_vo = pending
            texts = declared_dialogue_texts(line)
            records.extend((speaker, dialogue, is_vo) for dialogue in texts if dialogue)
            pending = None
    return records


def dialogue_partition_fact_ids_246(source: Any, facts: Any) -> set[str]:
    """Preserve the frozen 2.4.6 partition behavior for read-only validation."""
    records = dialogue_records_246(source)
    dialogue_facts = [
        fact
        for fact in as_list(facts)
        if isinstance(fact, dict) and clean_text(fact.get("type")) == "dialogue"
    ]
    if len(dialogue_facts) < 2 or not records:
        return set()

    memo: dict[tuple[int, int], tuple[tuple[int, int], ...] | None] = {}

    def solve(fact_index: int, first_record_index: int) -> tuple[tuple[int, int], ...] | None:
        if fact_index == len(dialogue_facts):
            return ()
        key = (fact_index, first_record_index)
        if key in memo:
            return memo[key]

        for record_index in range(first_record_index, len(records)):
            source_speaker, source_text, source_is_vo = records[record_index]
            assembled = ""
            for end_index in range(fact_index, len(dialogue_facts)):
                fact = dialogue_facts[end_index]
                fact_speaker = normalize_dialogue_speaker(fact.get("speaker"))
                fact_is_vo = clean_text(fact.get("delivery")) in {"vo", "offscreen"}
                if (source_speaker and fact_speaker != source_speaker) or fact_is_vo != source_is_vo:
                    break
                assembled += normalize_script_text(fact.get("text")).strip()
                if not source_text.startswith(assembled):
                    break
                if assembled == source_text:
                    remainder = solve(end_index + 1, record_index + 1)
                    if remainder is not None:
                        memo[key] = ((fact_index, end_index),) + remainder
                        return memo[key]
                    break
    ...
    if groups is None:
        return set()
    return {
        clean_text(dialogue_facts[index].get("fact_id"))
        for start_index, end_index in groups
        if end_index > start_index
        for index in range(start_index, end_index + 1)
        if clean_text(dialogue_facts[index].get("fact_id"))
    }


def _dialogue_colon_index(line: str) -> int:
    depth = 0
...
def _strip_trailing_direction(label: str) -> tuple[str, str]:
    value = label.rstrip()
    pairs = (("（", "）"), ("(", ")"))
    for opening, closing in pairs:
        if not value.endswith(closing):
            continue
        start = value.rfind(opening)
        if start >= 0:
            return value[:start].rstrip(), value[start + 1:-1]
    return value, ""


def _strip_voice_marker(label: str) -> tuple[str, str]:
    value = label.rstrip()
    patterns = (
        (r"^(.*?)(?:（VO）|\(VO\)|VO)$", "vo"),
        (r"^(.*?)(?:画外声)$", "offscreen"),
    )
    for pattern, delivery in patterns:
        match = re.match(pattern, value, re.IGNORECASE)
        if match:
            return match.group(1).rstrip(), delivery
    return value, "onscreen"


def strict_dialogue_label(
    line: str,
    known_speakers: set[str] | None = None,
) -> tuple[str, str, int] | None:
    colon_index = _dialogue_colon_index(line)
    if colon_index < 0:
        return None
    label = line[:colon_index].strip()
    if not label:
        return None
    speaker, delivery = _strip_voice_marker(label)
    if delivery == "onscreen":
        without_direction, _direction = _strip_trailing_direction(label)
        speaker, delivery = _strip_voice_marker(without_direction)
    speaker = normalize_dialogue_speaker(speaker)
    if not speaker:
        return None
    if known_speakers and speaker not in known_speakers:
        return None
    return speaker, delivery, colon_index + 1


def strict_dialogue_records(
    value: Any,
    known_speakers: set[str] | None = None,
    ...
    for raw_line in text.split("\n"):
        ...
        if not line:
            continue
        label = strict_dialogue_label(line, known_speakers)
        if label is not None:
            speaker, delivery, declared_start = label
            declared = line[declared_start:].strip()
            texts = declared_dialogue_texts(declared)
            records.extend((speaker, dialogue, delivery) for dialogue in texts if dialogue)
            pending = (speaker, delivery) if not declared else None
            continue
        if pending is not None:
            speaker, delivery = pending
            texts = declared_dialogue_texts(line)
            records.extend((speaker, dialogue, delivery) for dialogue in texts if dialogue)
            pending = None
    return records
...
def dialogue_records(value: Any) -> list[tuple[str, str, bool]]:
    return [
        (speaker, dialogue, delivery in {"vo", "offscreen"})
        for speaker, dialogue, delivery in strict_dialogue_records(value)
    ]


def dialogue_candidates(value: Any) -> set[str]:
    records = strict_dialogue_records(value)
    if records:
        return {dialogue for _speaker, dialogue, _delivery in records if dialogue}
    return {segment for segment in quoted_segments(value) if segment}


def dialogue_candidates_for_data(data: dict[str, Any], value: Any) -> set[str]:
    if uses_validation_integrity_guard(data):
        return dialogue_candidates(value)
    if uses_dialogue_partition_guard(data):
        return dialogue_candidates_246(value)
    return legacy_dialogue_candidates(value)


def dialogue_records_for_data(data: dict[str, Any], value: Any) -> list[tuple[str, str, bool]]:
    if uses_validation_integrity_guard(data):
        return dialogue_records(value)
    if uses_dialogue_partition_guard(data):
        return dialogue_records_246(value)
    return legacy_dialogue_records(value)


def dialogue_partition_fact_ids(
    source: Any,
    facts: Any,
    *,
    known_speakers: set[str] | None = None,
) -> set[str]:
    """Consume every source dialogue and dialogue fact in exact order without recursion."""
    records = strict_dialogue_records(source, known_speakers)
    fact_entries = [
        (index, fact)
        for index, fact in enumerate(as_list(facts))
        if isinstance(fact, dict) and clean_text(fact.get("type")) == "dialogue"
    ]
    if not records or not fact_entries:
        return set()

    approved: set[str] = set()
    fact_cursor = 0
    for source_speaker, source_text, source_delivery in records:
        assembled = ""
        previous_fact_position: int | None = None
        completed = False
        while fact_cursor < len(fact_entries):
            fact_position, fact = fact_entries[fact_cursor]
            if previous_fact_position is not None and fact_position != previous_fact_position + 1:
                return set()
            fact_speaker = normalize_dialogue_speaker(fact.get("speaker"))
            fact_delivery = clean_text(fact.get("delivery"))
            if fact_speaker != source_speaker or fact_delivery != source_delivery:
                return set()
            fact_text = normalize_script_text(fact.get("text"))
            if not fact_text:
                return set()
            assembled += fact_text
            if not source_text.startswith(assembled):
        ...
        if not completed:
            return set()
    if fact_cursor != len(fact_entries):
        return set()
    return approved


def known_dialogue_speakers(data: dict[str, Any], beat: dict[str, Any]) -> set[str]:
    scene_id = clean_text(beat.get("scene_id"))
    for scene_log in as_list(data.get("continuity_logs")):
        if isinstance(scene_log, dict) and clean_text(scene_log.get("scene_id")) == scene_id:
            speakers = declared_character_names(scene_log)
            if speakers:
                return speakers
    return {
        normalize_dialogue_speaker(fact.get("speaker"))
        for fact in as_list(beat.get("facts"))
        if isinstance(fact, dict)
        and clean_text(fact.get("type")) == "dialogue"
        and normalize_dialogue_speaker(fact.get("speaker"))
    }


def vo_dialogue_candidates(value: Any) -> set[str]:
    return {dialogue for _speaker, dialogue, delivery in strict_dialogue_records(value) if delivery != "onscreen"}


def speaker_bound_to_vo_246(value: str, known_speakers: set[str]) -> str:
    for speaker in sorted((item for item in known_speakers if item), key=len, reverse=True):
        escaped = re.escape(speaker)
        if re.search(rf"{escaped}\s*{_DIALOGUE_VO_PATTERN_246}", value, re.IGNORECASE):
            return speaker
        if re.search(rf"{_DIALOGUE_VO_PATTERN_246}\s*[-—：:]?\s*{escaped}", value, re.IGNORECASE):
            return speaker
    return ""


def vo_claim_for_quote_246(
    text: str,
    match: re.Match[str],
    known_speakers: set[str] | None = None,
) -> tuple[bool, str]:
    ...
    labels = list(_DIALOGUE_LABEL_RE_246.finditer(prefix))
    speaker = normalize_dialogue_speaker(labels[-1].group("speaker")) if labels else ""
    marked = bool(_VO_MARKER_RE_246.search(line))
    if marked:
        return True, speaker_bound_to_vo_246(line, speakers) or speaker
    previous_end = line_start - 1
    if previous_end <= 0 or prefix.strip():
        return False, speaker
    previous_start = text.rfind("\n", 0, previous_end) + 1
    previous_line = text[previous_start:previous_end]
    if not _VO_MARKER_RE_246.search(previous_line):
        return False, speaker
    previous_labels = list(_DIALOGUE_LABEL_RE_246.finditer(previous_line))
    previous_speaker = speaker_bound_to_vo_246(previous_line, speakers) or (
        normalize_dialogue_speaker(previous_labels[-1].group("speaker")) if previous_labels else ""
    )
    return True, previous_speaker


def legacy_speaker_bound_to_vo(value: str, known_speakers: set[str]) -> str:
    for speaker in sorted((item for item in known_speakers if item), key=len, reverse=True):
        escaped = re.escape(speaker)
...
def legacy_vo_claim_for_quote(
    ...
    line = text[line_start:line_end]
    prefix = text[line_start:match.start()]
    labels = list(_LEGACY_DIALOGUE_LABEL_RE.finditer(prefix))
    speaker = normalize_dialogue_speaker(labels[-1].group("speaker")) if labels else ""
    marked = bool(_LEGACY_VO_MARKER_RE.search(line))
    if marked:
        return True, legacy_speaker_bound_to_vo(line, speakers) or speaker
    previous_end = line_start - 1
    if previous_end <= 0 or prefix.strip():
        return False, speaker
    previous_start = text.rfind("\n", 0, previous_end) + 1
    previous_line = text[previous_start:previous_end]
    if not _LEGACY_VO_MARKER_RE.search(previous_line):
        return False, speaker
    previous_labels = list(_LEGACY_DIALOGUE_LABEL_RE.finditer(previous_line))
    previous_speaker = legacy_speaker_bound_to_vo(previous_line, speakers) or (
        normalize_dialogue_speaker(previous_labels[-1].group("speaker")) if previous_labels else ""
    )
    return True, previous_speaker
...
def vo_claim_for_quote(
    ...
    line_end = line_end_value if line_end_value >= 0 else len(text)
    line = text[line_start:line_end]
    marker = re.search(r"(?:（VO）|\(VO\)|(?<![A-Za-z0-9])VO\b|画外声)", line, re.IGNORECASE)
    if not marker:
        return False, ""
    return True, speaker_bound_to_vo_246(line, speakers)


def vo_claim_for_quote_for_data(
    data: dict[str, Any],
    text: str,
    match: re.Match[str],
    ...
    if uses_validation_integrity_guard(data):
        return vo_claim_for_quote(text, match, known_speakers)
    if uses_dialogue_partition_guard(data):
        return vo_claim_for_quote_246(text, match, known_speakers)
    return legacy_vo_claim_for_quote(text, match, known_speakers)
...
def validate_dialogue_fidelity(data: dict[str, Any], shot: dict[str, Any], result: ValidationResult) -> None:
    ...
    allowed = dialogue_candidates_for_data(data, source)
    source_records = dialogue_records_for_data(data, source)
    ...
    for fact_text in dialogue_facts:
        if uses_validation_integrity_guard(data):
            if fact_text:
                allowed.add(fact_text)
        else:
            allowed.update(dialogue_candidates_for_data(data, fact_text))
            if fact_text and not quoted_segments(fact_text) and not re.search(r"[：:]", fact_text):
    ...
    for field_name in ("camera_main_image", "prompt"):
        ...
        for match in re.finditer(r'“([^”]*)”|"([^"]*)"', field_text):
            segment = match.group(1) if match.group(1) is not None else match.group(2)
            marked_vo, claimed_speaker = vo_claim_for_quote_for_data(
                data, field_text, match, known_speakers
            )
            if not marked_vo:
                continue
            if claimed_speaker:
...
def validate_data(
    ...
    *,
    ...
    validate_performance_chains_and_constraints(
        data,
        result,
        allow_pending_gate_b_exceptions=allow_pending_gate_b_exceptions,
    )
...
def gate_b_exception_candidates(
    data: dict[str, Any], batch_id: str | None
) -> list[dict[str, Any]]:
    """Collect structurally declared per-shot exceptions for human review."""
    ...
    def add(shot: dict[str, Any], kind: str, marker: str) -> None:
        token = f"shot:{shot.get('shot_no')}:{kind}"
        candidates[token] = {
            "shot_no": shot.get("shot_no"),
            "kind": kind,
            "marker": marker,
    ...
    for shot in shots:
        ...
        for chain_break in as_list(shot.get("chain_breaks")):
            if (
                isinstance(chain_break, dict)
                and is_json_integer(chain_break.get("after_step"), minimum=1)
                and clean_text(chain_break.get("reason")) in PERFORMANCE_CHAIN_BREAK_REASONS
                and "[动作链拆分]" in notes
            ):
                add(shot, "performance-chain-break", "[动作链拆分]")
        if isinstance(shot.get("source_reuse"), dict) and "[原文复用]" in notes:
            add(shot, "source-reuse", "[原文复用]")

    previous: dict[str, Any] | None = None
    for shot in shots:
        if previous is not None and clean_text(previous.get("scene_id")) == clean_text(shot.get("scene_id")):
            notes = clean_text(shot.get("notes"))
            previous_geometry = as_dict(previous.get("camera_geometry"))
            geometry = as_dict(shot.get("camera_geometry"))
            if clean_text(geometry.get("angle_delta_from_previous")) in {"same", "minor"} and "[跳切说明]" in notes:
                add(shot, "jump-cut", "[跳切说明]")
            same_axis = clean_text(previous_geometry.get("axis_id")) and clean_text(
                previous_geometry.get("axis_id")
            ) == clean_text(geometry.get("axis_id"))
            if same_axis and {
                clean_text(previous_geometry.get("axis_side")),
                clean_text(geometry.get("axis_side")),
            } == {"side_a", "side_b"} and "[越轴说明]" in notes:
                add(shot, "axis-cross", "[越轴说明]")
            previous_movement = parse_triad(clean_text(previous.get("camera_main_image")))[2]
            movement = parse_triad(clean_text(shot.get("camera_main_image")))[2]
            if {
                depth_motion_direction(previous_movement),
                depth_motion_direction(movement),
            } == {"push_in", "pull_out"} and "[反转动机]" in notes:
                add(shot, "axis-reversal", "[反转动机]")
        previous = shot
    return sorted(candidates.values(), key=lambda item: (int(item["shot_no"] or 0), item["kind"]))
...
def review_package_text(
    ...
    lines = [
        ...
        "## Gate B 逐镜例外",
        "",
        "| 镜号 | 类型 | 标记 | 状态 | 建议 approved_items token |",
        "| --- | --- | --- | --- | --- |",
    ]
    for candidate in gate_b_exception_candidates(data, batch_id):
        lines.append(
            "| "
            + " | ".join(
                escape_cell(candidate.get(key))
                for key in ("shot_no", "kind", "marker", "status", "token")
            )
            + " |"
        )
    lines.extend([
    ...
    for constraint in as_list(data.get("directorial_constraints")):
        if not isinstance(constraint, dict) or (
            uses_gate_workflow_guard(data)
            and not constraint_in_scene_scope(data, constraint, scene_scope)
        ):
            continue
    ...
    lines.extend(
        [
            ...
            "## 自动派生情绪与表演摘要",
            "",
            "本表由 emotion facts、performance_cues、environment_cues 与 continuity_updates 自动生成，不要求人工另写摘要。",
            "",
            "| 镜号 | 情绪事实 | 表演提示 | 环境反馈 | 连续性变化 |",
            "| --- | --- | --- | --- | --- |",
        ]
    )
    facts_by_id = fact_lookup(data)
    for shot in shots:
        emotion_facts = [
            f"{fact_id}:{one_line(facts_by_id[fact_id].get('text'))}"
            for fact_id in normalized_list(shot.get("covered_fact_ids"))
            if fact_id in facts_by_id and clean_text(facts_by_id[fact_id].get("type")) == "emotion"
        ]
        performance = [
            f"{clean_text(cue.get('character'))}:{one_line(cue.get('description'))}"
            for cue in as_list(shot.get("performance_cues"))
            if isinstance(cue, dict)
        ]
        environment = [
            f"{clean_text(cue.get('kind'))}/{clean_text(cue.get('basis'))}:{one_line(cue.get('description'))}"
            for cue in as_list(shot.get("environment_cues"))
            if isinstance(cue, dict)
        ]
        updates = [
            (
                f"{clean_text(update.get('entity_type'))}:{clean_text(update.get('entity'))}."
                f"{clean_text(update.get('field'))}={one_line(update.get('from'))}→{one_line(update.get('to'))}"
            )
            for update in as_list(shot.get("continuity_updates"))
...
def validate_gate_a_prerequisites(
    data: dict[str, Any], result: ValidationResult, *, batch_id: str | None
) -> None:
    """Reject a Gate-A payload that cannot satisfy its later first-shot contract."""

    target_scenes = _batch_scene_ids(data, batch_id)
    if target_scenes is None:
        target_scenes = {
            clean_text(log.get("scene_id"))
            for log in as_list(data.get("continuity_logs"))
            if isinstance(log, dict) and clean_text(log.get("scene_id"))
        }
    first_beat_fact_types_by_scene: dict[str, set[str]] = {}
    for beat in as_list(data.get("beats")):
        if not isinstance(beat, dict):
            continue
        scene_id = clean_text(beat.get("scene_id"))
        if scene_id in first_beat_fact_types_by_scene:
            continue
        first_beat_fact_types_by_scene[scene_id] = {
            clean_text(fact.get("type"))
            for fact in as_list(beat.get("facts"))
            if isinstance(fact, dict) and clean_text(fact.get("type"))
        }
    logs = {
        clean_text(log.get("scene_id")): log
        for log in as_list(data.get("continuity_logs"))
        if isinstance(log, dict) and clean_text(log.get("scene_id"))
    }
    for scene_id in sorted(target_scenes):
        log = as_dict(logs.get(scene_id))
        anchor_type = clean_text(log.get("first_shot_anchor_type"))
        characters = declared_character_names(log)
        if anchor_type in {"multi_character", "both"} and len(characters) < 2:
            result.error(
                f"Gate A 前置校验：{scene_id} 的 {anchor_type} 首镜要求至少2名台账角色。"
            )
        first_fact_types = first_beat_fact_types_by_scene.get(scene_id, set())
        if anchor_type in {"multi_character", "both"} and "position" not in first_fact_types:
            result.error(
                f"Gate A 前置校验：{scene_id} 的 {anchor_type} 首镜以后必须绑定 position fact，"
                "但本场第一个 Beat 中不存在 position。若原句同时包含站位与动作，必须拆成同源 "
                "position fact 与 action/emotion fact 后再送审。"
            )
        if anchor_type in {"space", "both"} and "space" not in first_fact_types:
            result.error(
                f"Gate A 前置校验：{scene_id} 的 {anchor_type} 首镜要求本场第一个 Beat 包含 space fact，"
                "用于建立可追踪的空间锚点。"
            )


def validate_gate_zero_stage(
    data: dict[str, Any], *, data_path: Path, workspace_root: Path
) -> ValidationResult:
    result = ValidationResult()
    validate_json_compatible(data, result)
    if result.errors:
        return result
    validate_metadata(data, result, strict_status=False, final_signoff=False)
    validate_project_lexicon(data, result)
    validate_batch_plan(data, result, required_collections=())
    validate_script_lock(data, result, data_path=data_path, workspace_root=workspace_root)
    return result


def gate_a_audit_data(data: dict[str, Any], batch_id: str | None) -> dict[str, Any]:
    """Keep approved dependency shots while hiding the target batch's shot layer."""

    audit_data = gate_b_audit_data(data, batch_id)
    if uses_gate_workflow_guard(data) and batch_id:
        target_scenes = _batch_scene_ids(data, batch_id) or set()
        audit_data["shots"] = [
            shot
            for shot in as_list(audit_data.get("shots"))
            if isinstance(shot, dict) and clean_text(shot.get("scene_id")) not in target_scenes
        ]
    else:
        audit_data["shots"] = []
    audit_data["validation_report"] = {}
    audit_data["directorial_constraints"] = [
        constraint
        for constraint in as_list(audit_data.get("directorial_constraints"))
        if isinstance(constraint, dict) and clean_text(constraint.get("freeze_gate")) == "GATE_A"
    ]
    return audit_data


def validate_gate_a_stage(
    data: dict[str, Any], *, batch_id: str | None, data_path: Path, workspace_root: Path
) -> ValidationResult:
    audit_data = gate_a_audit_data(data, batch_id)
    result = ValidationResult()
    validate_json_compatible(audit_data, result)
    if result.errors:
        return result
    validate_metadata(audit_data, result, strict_status=False, final_signoff=False)
    validate_project_lexicon(audit_data, result)
    validate_batch_plan(
        audit_data,
        result,
        required_collections=("continuity_logs", "beats"),
    )
    if as_list(as_dict(data.get("batch_plan")).get("batches")) and not approved_review(
        data, "GATE_0"
    ):
        result.error("Gate A 前置校验：分批任务必须先有当前内容有效的全局 GATE_0 approved。")
    if uses_gate_workflow_guard(data) and batch_id:
        for dependency in sorted(batch_dependencies(data).get(batch_id, set())):
            if not effective_gate_approved(data, "GATE_B", batch_id=dependency):
                result.error(
                    f"Gate A 前置校验：依赖批次 {dependency} 必须先有当前有效的 GATE_B approved。"
                )
    validate_script_lock(audit_data, result, data_path=data_path, workspace_root=workspace_root)
    continuity_logs = validate_continuity_logs(audit_data, result)
    target_scenes = _batch_scene_ids(data, batch_id) if batch_id else None
    validate_inheritance_dependencies(
        data,
        result,
        target_scenes=target_scenes,
        require_parent_gate_b=True,
    )
    if uses_gate_workflow_guard(data) and batch_id:
        final_documents = final_scene_documents_from_shots(
            continuity_logs, audit_data.get("shots")
        )
        validate_inherited_state_values(
            audit_data,
            continuity_logs,
            final_documents,
            result,
    ...
    collect_facts(audit_data, result)
    validate_fact_cut_contract(audit_data, result)
    validate_fact_text_preservation(audit_data, result)
    validate_performance_chains_and_constraints(
        audit_data,
        result,
        require_shot_resolution=False,
    )
    validate_must_isolate_density(audit_data, result)
    validate_gate_a_prerequisites(audit_data, result, batch_id=batch_id)
    return result


def gate_b_audit_data(data: dict[str, Any], batch_id: str | None) -> dict[str, Any]:
    """Build a read-only dependency snapshot for staged preflight validation."""
    ...
    if not any(clean_text(batch.get("batch_id")) == batch_id for batch in batches):
        return audit_data
    if uses_gate_workflow_guard(data):
        required_batch_ids = set(transitive_batch_dependencies(data, batch_id)) | {clean_text(batch_id)}
        included_batches = copy.deepcopy(
            [batch for batch in batches if clean_text(batch.get("batch_id")) in required_batch_ids]
        )
    else:
        target_index = next(
            index
            for index, batch in enumerate(batches)
            if clean_text(batch.get("batch_id")) == batch_id
        )
        included_batches = copy.deepcopy(batches[: target_index + 1])
    included_batch_ids = {clean_text(batch.get("batch_id")) for batch in included_batches}
    included_scene_ids = {
        scene_id
        for batch in included_batches
        for scene_id in normalized_list(batch.get("scene_ids"))
    }
    audit_data["batch_plan"] = {"batches": included_batches}
    audit_data["continuity_logs"] = [
        item
        for item in as_list(audit_data.get("continuity_logs"))
        if isinstance(item, dict) and clean_text(item.get("scene_id")) in included_scene_ids
    ]
    audit_data["beats"] = [
        item
        for item in as_list(audit_data.get("beats"))
        if isinstance(item, dict) and clean_text(item.get("scene_id")) in included_scene_ids
    ]
    audit_data["shots"] = [
        item
        for item in as_list(audit_data.get("shots"))
        if isinstance(item, dict) and clean_text(item.get("scene_id")) in included_scene_ids
    ]
    included_fact_ids = {
        clean_text(fact.get("fact_id"))
        for beat in as_list(audit_data.get("beats"))
        if isinstance(beat, dict)
        for fact in as_list(beat.get("facts"))
        if isinstance(fact, dict) and clean_text(fact.get("fact_id"))
    }
    ...
    audit_data["performance_chains"] = [
        chain
        for chain in as_list(audit_data.get("performance_chains"))
        if isinstance(chain, dict)
        and clean_text(chain.get("scene_id")) in included_scene_ids
    ]

    def constraint_is_in_scope(constraint: Any) -> bool:
        if not isinstance(constraint, dict):
            return False
        if uses_gate_workflow_guard(data):
            scope = constraint_scene_ids(data, constraint)
            return bool(scope) and scope.issubset(included_scene_ids)
        constraint_type = clean_text(constraint.get("type"))
        if constraint_type == "direct_handoff":
            return (
                clean_text(constraint.get("from_fact_id")) in included_fact_ids
                and clean_text(constraint.get("to_scene_id")) in included_scene_ids
            )
        fact_ids = set(normalized_list(constraint.get("fact_ids")))
        scene_ids = set(normalized_list(constraint.get("scene_ids")))
        shot_nos = set(as_list(constraint.get("shot_nos")))
        return (
            (not fact_ids or fact_ids.issubset(included_fact_ids))
            and (not scene_ids or scene_ids.issubset(included_scene_ids))
            and (not shot_nos or shot_nos.issubset(included_shot_nos))
        )

    audit_data["directorial_constraints"] = [
        constraint
        for constraint in as_list(audit_data.get("directorial_constraints"))
        if constraint_is_in_scope(constraint)
    ]
    audit_data["human_reviews"] = [
        review
        for review in as_list(audit_data.get("human_reviews"))
        if isinstance(review, dict)
        and (
            clean_text(review.get("gate")) == "GATE_0"
            or (
                clean_text(review.get("gate")) in {"GATE_A", "GATE_B"}
                and clean_text(review.get("batch_id")) in included_batch_ids
            )
        )
    ]
    gate_zero = latest_review(audit_data, "GATE_0")
    if gate_zero is not None:
        gate_zero["reviewed_hash"] = gate_review_hash(audit_data, "GATE_0")
    return audit_data


def validate_gate_b_stage(
    data: dict[str, Any], *, batch_id: str | None, data_path: Path, workspace_root: Path
) -> ValidationResult:
    result = ValidationResult()
    batches = {
    ...
    if batches and not approved_review(data, "GATE_0"):
        result.error("Gate B 前置校验：当前内容缺少有效的全局 GATE_0 approved。")
    if uses_gate_workflow_guard(data) and batch_id:
        for dependency in sorted(batch_dependencies(data).get(batch_id, set())):
            if not effective_gate_approved(data, "GATE_B", batch_id=dependency):
                result.error(
                    f"Gate B 前置校验：依赖批次 {dependency} 缺少当前有效的 GATE_B approved。"
                )
    if not approved_review(data, "GATE_A", batch_id=batch_id):
        result.error(
            f"Gate B 前置校验：当前内容缺少有效的 GATE_A"
            f"{':' + batch_id if batch_id else ''} approved。"
        )
    if result.errors:
        return result
    audit_data = gate_b_audit_data(data, batch_id)
    ...
    expected_gate_b_missing = re.compile(
        rf"^{re.escape(VERSION)} 缺少有效的 GATE_B(?::BT[0-9]{{2,}})? approved 人工审核记录。$"
    )
    result.errors.extend(
        message for message in audit.errors if not expected_gate_b_missing.fullmatch(message)
    )
...
def command_review_package(args: argparse.Namespace) -> int:
    ...
    if metadata_version(data) != VERSION or not uses_director_integrity_guard(data):
        ...
        extend_result(result, validate_current_skill_package())
    ...
    if audit.errors:
        ...
        print_result(result)
        return 1
    package_audit = ValidationResult(warnings=list(audit.warnings))
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(
        review_package_text(data, gate="GATE_B", batch_id=batch_id, result=package_audit),
        encoding="utf-8",
    )
    print(f"WROTE: Gate B review package ({package_audit.status}) to {output_path}")
    return 0
...
def command_build(args: argparse.Namespace) -> int:
    ...
    if metadata_version(original) != VERSION:
        ...
        print_result(result)
        return 1
    package_result = validate_current_skill_package()
...
def command_review_hash(args: argparse.Namespace) -> int:
    ...
    if gate not in GATES:
        result.error("review-hash --gate 必须为 GATE_0 / GATE_A / GATE_B / GATE_C。")
    batch_id = clean_text(args.batch_id) or None
    if uses_stage_integrity_guard(data) and not clean_text(getattr(args, "workspace_root", None)):
        result.error(f"{VERSION} review-hash 必须显式提供 --workspace-root 运行对应阶段前置校验。")
    batches = {
        clean_text(batch.get("batch_id"))
        for batch in as_list(as_dict(data.get("batch_plan")).get("batches"))
        if isinstance(batch, dict) and clean_text(batch.get("batch_id"))
    ...
    workspace_root = Path(args.workspace_root).resolve(strict=False)
    if gate == "GATE_0":
        stage_result = validate_gate_zero_stage(
            data,
            data_path=data_path,
            workspace_root=workspace_root,
        )
    elif gate == "GATE_A":
        stage_result = validate_gate_a_stage(
            data,
            batch_id=batch_id,
            data_path=data_path,
            workspace_root=workspace_root,
        ...
        stage_result = validate_gate_b_stage(
            data,
            batch_id=batch_id,
            data_path=data_path,
            workspace_root=workspace_root,
...
def command_validate(args: argparse.Namespace) -> int:
    ...
    if (
        not uses_gate_workflow_guard(data)
        and clean_text(as_dict(data.get("validation_report")).get("status")) == "NOT_RUN"
...
def make_parser() -> argparse.ArgumentParser:
    ...
    review_hash.add_argument(
        "--workspace-root",
        required=True,
        help=f"required for {VERSION}; runs the selected Gate preflight before emitting a hash",
    )
