#!/usr/bin/env python3
"""Validator/compiler for su-image9 v1.7.3 final prompts."""
...
def parse_blocks(page: Page) -> None:
    headers = list(re.finditer(r"(?m)^(STYLE_LOCK|CANVAS_LOCK|REFERENCE_LOCK|CONTINUITY_LOCK|PANEL_TASKS(?:\s+(?:P\d{2}-P\d{2}|PANEL-1\s+to\s+PANEL-9))?|NEGATIVE_LOCK):\s*$", page.text))
    blocks: dict[str, str] = {}
...
def render_page(page: Page) -> str:
    ...
    for name in order:
        if name in page.blocks:
            suffix = " PANEL-1 to PANEL-9" if name == "PANEL_TASKS" else ""
...
def validate_g0(pages: list[Page], compiled_text: str, panel_plan: dict[str, Any]) -> None:
    for page in pages:
        ...
        if re.search(r"(?m)^PANEL_TASKS\s+P\d{2}-P\d{2}:\s*$", page.text):
            page.findings.append(issue("G0-11", "fail", page.name, "PANEL_TASKS Pxx-Pxx", "old panel task header naming is not allowed"))
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compile and validate su-image9 v1.7.3 prompts.")
