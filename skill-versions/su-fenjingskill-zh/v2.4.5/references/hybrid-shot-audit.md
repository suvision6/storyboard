...
本文件所有规则均指 2.4.5 本版规则。事实级切点字段的结构定义以 `continuity-shot-data.md` 为唯一出处，本文件只规定审计与拆合判断。项目专有字面词已迁入可选 `references/project-notes.md`；只有显式复制到 `metadata.project_lexicon` 的词才参与当前数据校验。
...
- **拆合冲突仲裁**：`must_isolate` 仍高于普通合并建议，但不能自动绕过 2.4.5 机位几何与表演链规则。若必拆后形成同轴 `same | minor` 相邻镜，必须重设为等价 30 度以上机位变化；只有明确有意跳切可用 `[跳切说明]` + `shot:<镜号>:jump-cut`。旧 `[必拆相邻]` 只保留历史合同语义。
...
以下指标写入 `validation_report.hybrid_audit` 与 `director_integrity_audit.rhythm`，只允许 WARN，不允许 FAIL；2.4.5 的节奏越界**不在自动白名单**，必须由人工在 `warn_resolutions` 处置：
...
低 cut/min、低短镜比例继续提示压缩风险；高于 12.5 cut/min、短镜比例高于 18%、长镜比例低于 40% 提示过度拆镜。不得为了让参考指标好看而机械增删镜头；任何因指标调整的拆合都必须能解释为画面信息或情绪节奏需要，并记入 `revision_log`。
