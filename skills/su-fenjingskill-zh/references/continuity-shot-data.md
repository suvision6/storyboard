...
6. `dialogue` 事实的 `text` 只写需要逐字保留的对白原文，不含角色名前缀或说明。
...
`batch_id` 格式为 `BT[0-9]{2,}`，数字部分只用 ASCII `0-9`，当前合同不额外禁止 `BT00` 或超过两位，且必须唯一；每个场景只能属于一个批次；`expected_shot_count` 必须是正整数；`depends_on` 只能引用已存在批次且依赖图无环。所有 Beat 与 Shot 必须归属已登记场景及批次。空对象或空 `batches` 不是合法长输入计划，也不能用来绕过 Gate 0。
...
```json
{
  ...
  "facts": [
    {"fact_id": "B001-F01", "type": "action", "text": "必须拍到的事实", "cut_priority": "normal", "cut_reasons": [], "cut_group": "B001-action", "cut_category": "action", "cut_moment_id": "B001-action-moment"}
  ]
}
```
...
Gate 0/A/B 使用各阶段 hash；Gate C `reviewed_hash` 必须等于 `canonical_data_hash`。hash 范围与算法以 `SKILL.md` 为准。`[安全镜]`、`[反转动机]`、`[必拆相邻]` 的 `approved_items` token 固定为 `shot:<shot_no>:safety`、`shot:<shot_no>:axis-reversal`、`shot:<shot_no>:required-adjacent`；泛化批准无效。2.4.3 预签发要求 Gate A/B（分批时含 Gate 0）最新轮次批准且 hash 匹配；`--final-signoff` 额外要求 Gate C 最新轮次批准且 hash 匹配。
...
2.4.3 的 build/validate 必须传 `--workspace-root` 与 `--report`。build 将 JSON、Markdown、Excel、report 全部写入同目录临时文件，四份生成并自检成功后再原子替换；任一步失败都保留既有正式文件，不回写输入 JSON。Markdown/Excel 必须使用同一 canonical 单元格格式化规则，保证换行、字面量 `<br>`、数值和以 `= + - @` 开头的文本可逆且不被当作公式。
