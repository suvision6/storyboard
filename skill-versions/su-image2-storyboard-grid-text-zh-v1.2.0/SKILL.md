...
version: "1.2.0"
description: 无参考图时使用。根据剧本文字、纯剧情段落、文字版分镜、Markdown/Excel 表格、局部镜号或表格截图转写内容，推演适用于 Image2/gpt-image-2 的 3x3 九宫格黑白分镜提示词 Markdown。分析表使用中文，最终 Image2 可复制提示词使用英文主导。必须把第 1 格作为主平面锚定格，锁定室内/外景空间、家具、地形、门窗、道路、人物起点和运动路径；必须锁定对视轴线、反打肩位和银幕左右关系，防止反打越轴。不得新增剧情事实，不要求补参考图，不修改 su-fenjingskill-zh 主表、Prompt 列、Storyboard 列、Excel 或校验脚本。
---
...
1.2.0 强化第 1 格主平面锚定：所有连续九宫格必须优先让第 1 格建立室内/外景空间、家具、地形、门窗、道路、人物起点和运动路径。后续格只能从第 1 格裁切、推进、反打、俯拍或换侧面角度，不得重新布置空间。
...
- 第 1 格主平面锚定优先级高于原始镜头景别，但低于“不新增剧情事实”和“不提前揭示隐藏对象”。
...
4. 建立第 1 格主平面锚定：无论原始第一节点是否为近景/特写，都先用全景、大全景、俯视全景或主观全景锁定可公开空间。
5. 建立必要锁定表：第1格主平面锚定、主体空间朝向、室内平面布局、前序继承、门槛边界、对象足迹、机位编号、对视轴线与反打锁定、摄影机朝向、对象可见性。
6. 从来源镜头或剧情中选取九个可见节点，写入 `九格取舍表`。
7. 生成英文主导的 `Image2 可复制提示词`。
8. 静默自检比例、风格、文字禁令、第 1 格锚定继承、空间连续性、对象可见性和仿真词清洗。
...
- 室内场景必须在第 1 格建立平面布局：门、窗、墙、床、桌、沙发、梳妆台、柜体、通道、关键道具。
- 外景必须在第 1 格建立地形/道路/建筑/院门/树木/车辆/人物起点等空间锚点。
- 近景、特写、反打、俯拍和主观视角只能从第 1 格主平面推导，不能每格重新布置。
...
所有连续九宫格必须输出。第 1 格是主平面锚定格，不等同于剧情第一特写。
...
- `第1格锚定景别` 必须是全景、大全景、俯视全景或主观全景；原始第一节点是近景/特写时，也要改写为可公开空间锚定。
- `必须建立的固定锚点` 写清门、窗、墙、床、桌、梳妆台、道路、院门、建筑、树、车、人物起点、运动路径等。
- `允许隐藏/暂不出现对象` 写清尚未出现的人物、道具、灵异对象；这些对象不得提前画出，只能锚定空位置或区域。
- `后续继承规则` 必须说明后续格只能从第 1 格裁切、推进、反打、俯拍或换侧面角度。
- `禁止提前揭示或重建` 必须说明不得提前揭示隐藏对象，不得重建新房间、新街道、新家具、新门窗或新地形。
...
同一场景连续九宫格必须输出。每个后续格必须写清继承第 1 格哪些锚点、哪些只允许裁切、哪些不得重画。
...
Panel 1: wide establishing master layout anchor, source-supported visible space only, no premature reveal of hidden characters or props.
...
- 若格数、比例、黑白草图风格、后续宫格没有继承第 1 格室内/外景锚点、家具/地形/门窗/道路重排、近景补出新背景、空间连续性、对象可见性、画内文字、反打越轴、人物左右互换、过肩方向不明任一失败，收紧提示词并重生一次；最多连续重生两次。
...
- 是否输出 `第1格主平面锚定表`，并确认第 1 格是全景、大全景、俯视全景或主观全景锚定格。
