...
- `metadata.version` 为 `2.3.4` 时，`metadata.rule_revision` 必须是 `2.3.4-overcompression-guard-2026-06-30`；为 `2.3.5` 时，必须是 `2.3.5-adjacent-motion-guard-2026-06-30`。缺失或不匹配视为旧合同数据并失败。
- `metadata.version` 为 `2.3.5` 时，同场相邻镜头不得出现无动机轴向反转：推进类运动后不得直接接拉出/拉远类运动，反向组合也不得出现。
