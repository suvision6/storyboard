...
本文件所有规则均指 2.4.0 本版规则。事实级切点字段的结构定义以 `continuity-shot-data.md` 为唯一出处，本文件只规定审计与拆合判断。项目专有的切点清单（特定角色 VO、特定道具、剧集倾向）一律存放在 `references/project-notes.md`，位阶低于本文件。
...
- **拆合冲突仲裁**（与 `SKILL.md` 仲裁顺序第 6 条一致）：`must_isolate` 拆分要求高于相邻镜头信息增量合并要求。因必拆产生的低增量相邻镜组，备注写 `[必拆相邻]`，信息增量校验对该镜组豁免，并自动进入 Gate B 人工复核清单。
- 同场相邻镜头**默认禁止轴向反转**：推进类运动后不得直接接拉出/拉远类运动，反向组合同样禁止。确有叙事动机时，备注写 `[反转动机]` 加一句话理由，校验降为 WARN 并进入 Gate B 复核清单；无标注即 FAIL。
...
默认 `must_isolate` 的通用类型（项目实例清单见 `project-notes.md`）：
...
- 剪辑确需无事实安全镜时，只能在 Gate B 由人工批准添加，备注写 `[安全镜][人工批准]`，并在 `human_reviews` 的 GATE_B 记录中留有对应批准意见。
- 校验器对备注含 `[安全镜][人工批准]` 且有对应 GATE_B `approved` 记录的镜头，豁免"每镜必须覆盖 ≥1 事实"检查；无批准记录的 `[安全镜]` 标注视为 FAIL。
...
本文件不包含任何项目专有内容。特定剧集的段落倾向、角色 VO 清单、关键道具节点等，一律写入 `references/project-notes.md`；该文件规则位阶低于本文件与 `SKILL.md`，冲突时以 skill 层为准。