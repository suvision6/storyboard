...
`su-image9` 是 3x3 九宫格黑白分镜提示词独立入口。它保留九宫格横版容量，但补入 `su-image7` 中更强的空间、车辆、反打和验收规则。不要转用 `su-image2-storyboard-grid-zh`。
...
- 画内禁止任何文字；Markdown 中的 `Panel 1` 到 `Panel 9` 只供阅读和复制，不是画面内容。
...
目检顺序固定为：整体横版 16:9、3x3 九格、每格 16:9、无画内文字、Panel 1 主平面锚定、固定物几何继承、车辆局部坐标、座位-车窗邻接、车内摄影机占位、车内外同侧窗口、反打轴线、对象可见性。任一失败，收紧提示词并重生一次；最多连续重生两次。
