...
当前版本：2.4.5。
当前规则修订：`2.4.5-director-shot-integrity-guard-2026-07-18`。
...
1. 技能根目录下 `VERSION` 为 `2.4.5`。
2. `SKILL.md` 当前规则修订为 `2.4.5-director-shot-integrity-guard-2026-07-18`。
...
本文件其余部分的所有规则均指 2.4.5 本版规则。`build` 只允许生成 2.4.5 数据；旧版本仅允许 `validate` 只读校验。缺失、未知、未来版本或与版本不匹配的 `rule_revision` 一律 FAIL，不得按最近版本猜测执行。旧数据兼容只按下表处理：
...
| 2.4.3 | 只读兼容：保留技能相对入口、真实源文、内容 hash 与人工门禁的严格合同校验。 |
| 2.4.4 | 只读兼容：保留 2.4.3 合同与情绪表演守卫，不得用于新建交付。 |
| 2.4.5 | 本版全部规则生效；新增导演机位几何、动作链、人物映射、原文复用、审核约束与过度拆镜守卫。 |

### 受保护审计字段统一清单

Prompt 派生阶段及一切后置阶段禁止修改以下字段：

`metadata.project_lexicon`、`beat_order`、`shot_type`、`split_reason`、`insert_priority`、`long_take_support`、`cut_priority`、`cut_reasons`、`cut_group`、`cut_category`、`cut_moment_id`、`inherits_from`、`inherited_states`、`diverged_states`、`spatial_axes`、`continuity_updates`、`covered_fact_ids`、`beat_ids`、`duration_seconds`、`duration_breakdown`、`long_take`、`script_lock`、`source_span`、`source_spans`、`source_reuse`、`camera_geometry`、`performance_cues`、`action_progressions`、`environment_cues`、`chain_breaks`、`performance_chains`、`directorial_constraints`，以及 action/emotion fact 的 `performers`、dialogue fact 的 `speaker` / `addressees` / `delivery`。
...
| **Gate B** | 导演主表前 6 列完成后 | 6 列主表、审计字段摘要、情绪表演摘要、仲裁清单（`[必拆相邻]` / `[反转动机]` / 长镜复核 / `[安全镜]` 申请）、时长分布 | 前 6 列 + 全部受保护审计字段 |
...
6. 每条审核记录都必须显式写 `approved_items`；只有 Gate B 可写非空数组，Gate 0/A/C 必须写 `[]`。旧 token 保留 `safety | axis-reversal | required-adjacent`；2.4.5 新增 `jump-cut | axis-cross | performance-chain-break | source-reuse`。格式固定为 `shot:<shot_no>:<token>`，并分别要求 `[跳切说明]`、`[越轴说明]`、`[动作链拆分]`、`[原文复用]` 及对应结构化字段；泛化批准无效。
...
`batch_id` 必须唯一；每个场景只属于一个批次；`depends_on` 只能引用既有批次且不得成环。所有 Beat/Shot 必须归属已登记场景与批次。`expected_shot_count` 仅是容量规划参考，不是拆镜目标、下限或上限，不得为命中数量而拆合。单批任务使用 `null`；空对象或空 `batches` 不是合法长输入计划，也不能触发 Gate 0 豁免。
...
3. **建立连续性台账**：每场分配机器键 `scene_id`（S01、S02…），显示名仅用于展示；JSON 内一切场景引用只用 `scene_id`。按场景锁定空间轴线、固定物、角色初始站位、道具状态、声音来源、现实层级，填写 `first_shot_anchor_type`。2.4.5 用 `spatial_axes[]` 给每条主要视线/行动/空间轴分配 `AXxx`，写明 `axis_type`、两端点与两侧锚点；多人锚定场景至少一条。
...
4. **拆分 Beat 与事实**：先生成本场空间锚定事实与站位锚定事实，再按场景目标、动作/台词关系、空间变化、道具变化和现实层级拆分可表演 Beat。`space_anchor_fact` / `blocking_anchor_fact` 是职责名称，不是新增 JSON 字段或 fact type：分别用现有 `type: "space"` / `type: "position"` 表达，并填写既有切点字段。Action/emotion fact 必须登记非空 `performers[]`；纯物体、环境、声光或现实层变化改用 prop/space/sound/reality，而不能伪装为无表演者 action。Dialogue fact 另登记 `speaker`、`addressees[]`、`delivery: onscreen | offscreen | vo`，所有人物必须精确命中本场台账。
   - 将“同一人物动作→反应→台词”或等价连续表演登记为顶层 `performance_chains[]`；默认整条链同镜，`recommended`/`insert_priority` 不能自动拆开。校验器还会对连续、可唯一归属同一角色的 action/emotion/dialogue facts 做兜底推断：若它们未共镜且未登记 chain，直接 FAIL，不能靠省略数组绕过。只有不可兼容的必拆事实、现实/时间/空间断裂或明确编辑强调，才可在后镜登记 `chain_breaks` 并由 Gate B 精确批准。
   - 人工纠正形成的“同镜、同 Beat、禁止字面元素、跨场直接承接”登记为顶层 `directorial_constraints[]`，含稳定 `constraint_id`、`type`、`freeze_gate` 与 `origin {gate, round}`；后续不得靠重新生成丢失。
...
8. **锁定导演主表前 6 列并自检**：
   ...
   - 每镜填写 `camera_geometry`：轴线 ID/所在侧、机位空间位置与朝向、相对上镜的等价角度变化、主体、POV 与人物屏幕方向。`primary_subjects` 必须映射到本镜可见/POV人物，或本场已登记且在主画面/可见道具中出现的实体。相邻同轴 `same | minor` 视为小于等价 30 度变化；改变景别不能代替改机位，默认合并或重设。有共同可见人物、共同主体，或同轴且 `camera_position` 未变时不得写 `not_comparable`；仅有意跳切可用 `[跳切说明]` + Gate B token。
   - 同一轴线 `side_a ↔ side_b` 直接切换默认越轴 FAIL；只有 `[越轴说明]` + Gate B token 可降 WARN。`axis-reversal` 仍只表示纵深推/拉方向反转，不得与 180 度越轴混用。人物屏幕方向反转必须有 position/facing 连续性更新。
   - 主观镜必须登记 `pov_character`；非主观镜必须为 `null`。画面描述中的角色必须映射到 `visible_characters`、`offscreen_characters` 或 POV；对白说话人与受话人必须按 delivery 落到相应集合。VO/画外对白不得单独成为只覆盖 dialogue fact 的镜头，必须与可见动作、反应、空间或其他非对白事实同镜作为画面支撑。
   - 每镜填写有事实证据的 `performance_cues`（有可见人物时 1–3 项）、`action_progressions`（状态动作写起点/过程/终点）、`environment_cues`（声音/光线/接触依据）。人物表演 cue 只能引用 action/emotion/dialogue/position 事实，不能借 space/prop 等环境事实洗证据；cue.character 必须分别命中 performers、speaker，或被 position fact.text 明确点名。每个已覆盖 action/emotion fact 的 `performers` 必须映射到本镜可见/画外/POV人物；可见表演者必须有引用该 fact 的 cue。描述必须逐字存在于主画面，不得成为脱离画面的元数据。
   - 相邻镜头完全相同的 `source_span(s)` 默认视为重复拆镜；只有结构性不可合并时登记 `source_reuse {from_shot_no, reason}`、`[原文复用]` 并经 Gate B 精确批准。
   ...
   - 生成逐镜情绪表演摘要：列出情绪依据、可见/可听表演、前后镜连续性、声音/环境反馈依据与保留/修改结论。该摘要只进人工审核包，不新增 JSON 字段或 `approved_items` token。
   - 在 Gate B 前确定从 1 开始连续的最终镜号；Gate B 批准后禁止重排镜号。
9. **【Gate B：主表锁定审核】** 先用 `review-package` 生成含冻结约束、场景轴线、相邻机位、表演链、人物映射、原文复用与节奏统计的审核包，再提交 Gate B 内容 hash 并停机等待。批准后前 6 列与审计字段冻结。
...
13. **【Gate C：交付签发审核】** 提交校验报告、WARN 处置表、最终三文件清单与 Gate C 内容 hash，停机等待签发。
    ...
    - 仅白名单 WARN（reference missing、`[合理补足]`）允许 `resolved_by: "auto_whitelist"`；2.4.5 的节奏越界、跳切、越轴、动作链拆分与原文复用一律 `resolved_by: "human"`。
...
- 只根据 `locked_text`、当前 Beat/fact 或当前剧情语境中已成立的情绪设计表演；不得新增情绪阶段、冲突、道具或剧情事实。
- 每镜从眼神/面部、呼吸/声音、姿态/手部中选 1–3 个高信号表现；不强制四层动作、峰值失控或持续运动。当原文支持麻木、屏息、压抑或僵住时，静止是合法表演。
- 人物位移、道具操作或不可逆状态变化必须写清起点、过程、终点；普通微动作不强制三段式。
- 对白引号内逐字保真，音量、音质、语速和呼吸等表演描述写在引号外。倾听者反应只能分配给当前镜头的可见角色，且不得因短反应自行碎切。
- 环境反馈只允许来自原文/fact，或当前动作直接、不可避免且不改变剧情状态的脚步、衣物摩擦、接触碰撞等。破裂、掉落、移位或状态改变必须有 fact 依据并登记连续性更新。
- 光线只保留原文或已有场景依据中可见的方向、明暗和冷暖结果；不得新增导演风格、美学流派、色温数值或新光源。
- 2.4.5 的非对白描述不得使用毫米、厘米、米、Hz/K 或精确角度等执行数值；改用一臂距离、隔桌相对、低角度、冷暖倾向和可见结果。镜头秒数、年龄、画幅比例与原样对白中的数值不受此限制。`angle_delta_from_previous` 的 `same | minor | substantial` 是等价区间，不写伪精确角度。
...
- 同场相邻镜头必须有明确画面信息增量。2.4.5 以 `camera_geometry` 为机器事实：同一轴线下相对变化为 `same | minor` 时，不论景别是否变化都默认 FAIL；`substantial` 必须伴随 `camera_position` 改变。
- 只有明确有意的跳切可用 `[跳切说明]` + `shot:<n>:jump-cut`；旧 `[必拆相邻]` 仅保留历史版本语义，不再绕过 2.4.5 的机位几何检查。
- 同轴 `side_a` 与 `side_b` 直切是 180 度越轴；用 `[越轴说明]` + `shot:<n>:axis-cross` 才可降为人工 WARN。屏幕左右/向镜头与离镜头方向无连续性更新却反转，一律 FAIL。
...
`[合理补足]`、`[时长估算]`、`[长镜头]`、`[保留理由]`、`[不可拆说明]`、`[景别跨度]`、`[非连续Beat]`、`[必拆相邻]`、`[反转动机]`、`[安全镜]`、`[人工批准]`、`[声音]`、`[reference missing]`、`[跳切说明]`、`[越轴说明]`、`[动作链拆分]`、`[原文复用]`
...
- `画面内容` 必须保留已锁定主画面中的可拍动作、表情、呼吸、合法声音质感与环境反馈；不得在派生时二次补写或压缩。
- 不得把六段式总提示词、1900 字总上限、缩减台词、自动增强冲突、全局风格或“禁止”指令引入第 7 列。
...
- 2.4.5 每场 `spatial_axes`、每镜 `camera_geometry` / `performance_cues` / `action_progressions` / `environment_cues` / `chain_breaks`，以及顶层 `performance_chains` / `directorial_constraints` 都是必需结构；允许为空的数组也必须显式存在。完整字段 schema 以 `continuity-shot-data.md` 为准。
...
2.4.5 的 `build` 与 `validate` 都必须显式传入 `--workspace-root` 和 `--report`；2.4.4 及更旧版本只能执行只读 `validate`。每次提交 Gate 审核前，先用只读 `review-hash` 生成当前审核范围的 hash；该命令不修改文件，也不代表内容已通过校验：
...
& <bundled-python> <skill-root>/scripts/storyboard_delivery.py review-package --workspace-root <workspace-root> --data <片名>.shot_data.json --gate GATE_B --output <片名>.gate-b-review.md
```
...
2.4.5 校验器沿用 2.4.4 的真实源文、引用、人工门禁、hash、事实覆盖、时长、WARN、连续性与情绪表演守卫，并新增导演机位几何、人物映射、动作链、直接约束、原文复用和过度拆镜检查。全片软目标为 cut/min 10.5–12.5、≤3 秒镜比例 10%–18%、≥6 秒镜比例 40%–55%；高 cut/min、高短镜比或低长镜比只触发 WARN，但必须由人工处置，不能自动白名单。`source_json_hash` 等于 `canonical_data_hash`：排除 `validation_report`、`human_reviews`、`metadata.revision_log`，包含 `warn_resolutions`；Gate C `reviewed_hash` 必须等于该 hash。strict validate 重算真实结果，不接受陈旧或自称 PASS 的报告。
...
- AXxx 轴线、机位所在侧/朝向/位置、等价角度变化、屏幕方向与主观 POV 完整；相邻镜无未批准的小角度跳切或越轴。
- 动作→反应→台词链默认同镜；人物与对白映射、表演/动作/环境证据、原文复用和人工冻结约束全部通过。
...
| 失败项 | 本版机器约束 |
| --- | --- |
| 同轴同角度连续切镜 | `camera_geometry` + 等价 30 度变化；仅换景别无效 |
| 越轴与屏幕方向漂移 | AXxx 所在侧、screen direction 与 continuity update 联检 |
| 动作、反应、台词被机械拆开 | `performance_chains` 默认同镜，拆分需 Gate B 精确例外 |
| 人名映射错误、VO 无画面支撑 | dialogue speaker/addressees/delivery 与可见、画外、POV 集合联检 |
| 同一原文重复成镜 | 相邻 span 去重；`source_reuse` 仅限结构性例外 |
| 人工纠正后再次丢失 | `directorial_constraints` 在 Gate A/B 冻结并进入审核 hash |
| 过度碎切 | cut/min、短镜比、长镜比双向 WARN，必须人工处置 |
