...
本文件是 `su-image9` v2.0.1 的 canon 编译源。`SKILL.md` 正文内联副本是最终仲裁；本文件必须与正文锁块同步。v2.0.1 未改动锁块正文，但同步升级 canon-version 以标记规则、派生脚本与门禁行为变化。
...
`forbidden_prompt_tokens_extra` 只能追加，不能删减 canon 内置硬禁。v2.0.1 的硬禁扫描仅作用于 `CONTINUITY_LAYER` 与 `PANEL_LAYER`，锁块本身不参与扫描。
